...
This function allows the creation of a UF_KF_value object without an
assigned dimension.
...
/****************************************************************************
Create a Knowledge Fusion Dimensioned Number value.

A UF_KF_value object may be dimensioned. 
The dimension is determined by its measure.
For example, a UF_KF_value that represents a distance should
be holding a LENGTH measure.

If the dimension does not involve angles, then the input numerical value
would always be in the base units of the part.

If the dimension involves angles, such as UF_MEASURE_ANGULAR_VELOCITY, then
the input numerical value should be in degree-based units.
Example:
    For angular velocity, the number would be in degrees/s.

This function allows the creation of a UF_KF_value object with an
assigned dimension. If the numeric value is truly dimensionless, use 'Unitless'
...
Environment: Internal and External
See Also:
History: Released in NX1872
 ***************************************************************************/
extern UFUNEXPORT int UF_KF_make_number_with_measure
(
    double           data, /* <I>  Data of value */
    const char * measure,  /* <I>  The measure type of the value */
    UF_KF_value_p_t *value /* <OF> Optionally dimensioned Knowledge Fusion value.
                                   Free with UF_KF_free_rule_value.
                                   Do not free if being returned from a CFunc. */
);
...
/****************************************************************************
Create a Knowledge Fusion Dimensioned Number value with a unit

A UF_KF_value object may be dimensioned. 
The dimension is determined by its UF_UNIT_MEASURE_TYPE.
For example, a UF_KF_value that represents a distance should
be holding a LENGTH measure.

In addition, if the UF_KF_value object is used as a list item, it may
hold additional information, such as a unit type, which is used when the 
list item is displayed. For non-list number values, the display of the
values is controlled by the owning expression or KF attribute, but list
items need to own the display information.

If the dimension does not involve angles, then the numerical value
would always be in the base units of the part.

If the dimension involves angles, such as ANGULAR_VELOCITY, then
the input value should be in degree-based units.
Example:
    For angular velocity, the number would be in degrees/s.

This function allows the creation of a UF_KF_value object with an
assigned dimension and display unit. If the numeric value is truly dimensionless, use NULL_TAG
as the input unit.

Note: The UF_KF_value object will be given a dimension that is determined by the given units.
      The UF_KF_value object can be used for non-list purposes, but in such cases, the
      given units are ignored, while the dimension is still acknowleged.
...
History: Released in NX1872
 ***************************************************************************/
extern UFUNEXPORT int UF_KF_make_number_with_unit
(
    double           data, /* <I>  Data of value */
    const char      *unit, /* <I>  The display unit (provides also dimensionality). */
    UF_KF_value_p_t *value /* <OF> Optionally dimensioned Knowledge Fusion value.
                                   Free with UF_KF_free_rule_value.
                                   Do not free if being returned from a CFunc. */
);
...
A UF_KF_value object may be dimensioned. For example, a value that represents
a distance should generally be holding a LENGTH measure.

If the dimension does not involve angles, then the returned numerical value
will always be in the base units of the part. If the dimension involves
angles, then the numerical value will be returned degree-based.
Example:
  For angular velocity, the returned numerical value would be in degrees/s.
...
/****************************************************************************
Get the value and measure of a Number Knowledge Fusion structure.

A UF_KF_value object may be dimensioned. 
The dimension is determined by its measure.
For example, a UF_KF_value that represents a distance should generally 
be holding a LENGTH measure.
...
History: Released in NX1872
 ***************************************************************************/
extern UFUNEXPORT int UF_KF_ask_number_with_measure
(
    UF_KF_value_p_t  value,  /* <I> Knowledge Fusion data structure */
    double          *data,   /* <O> Data of value */
    char           **measure /* <O> The measure of the value
                                    Free with UF_free. */
);

/****************************************************************************
Get the value and display unit of a Number Knowledge Fusion structure.
...
hold additional information, such as a unit type, which is used when the 
list item is displayed. For non-list number values, the display of the
values is controlled by the LHS (left hand side) of the owning expression or KF attribute,
but list items need to own the display information.

If the dimension does not involve angles, then the returned numerical value
will always be in the base units of the part. If the dimension involves
angles, then the numerical value will be returned degreebased.
Example:
  For angular velocity, then the returned numerical value would be in degrees/s.

This function will return the value of UF_KF_value object as well
as its dimension. The numerical value is returned according to the
above rules, independent of any present display unit.
...
History: Released in NX1872
 ***************************************************************************/
extern UFUNEXPORT int UF_KF_ask_number_with_unit
(
    UF_KF_value_p_t  value, /* <I> Knowledge Fusion data structure */
    double          *data,  /* <O> Data of value */
    char           **unit   /* <O> Unit type */
);

/****************************************************************************
Convert a number from degree-based units to radian-based units

This is intended for implementations, such as of dfa functions,
where a value in degree-based units (like degrees/s) needs to be
converted into radian-based units (like radians/s).

The conversion applied depends on the angular dimension of the value.
For example, a value measured in degrees/s is multiplied by pi/180, while
a value measured in mV-s/degree is multiplied by 180/pi. Further, a value
measured in mm is left untouched, as it has no angular dimension.
...
History: Released in NX1872
*****************************************************************************/
extern UFUNEXPORT int UF_KF_convert_to_radian_base
(
    double data,           /* <I> The number to convert */
    const char * measure,  /* <I> The measure of the number */
    double *data_converted /* <O> The number converted to radian-based units */
);

/****************************************************************************
Convert a number from radian-based units to degree-based units

This is intended for implementations, such as of dfa functions,
where a value in radian-based units (like radians/s) needs to be
converted into degree-based units (like degrees/s).

The conversion applied depends on the angular dimension of the value.
For example, a value measured in radians/s is multiplied by 180/pi, while
a value measured in mV-s/radian is multiplied by pi/180. Further, a value
measured in mm is left untouched, as it has no angular dimension.
...
History: Released in NX1872
*****************************************************************************/
extern UFUNEXPORT int UF_KF_convert_to_degree_base
(
    double data,           /* <I> The number to convert */
    const char * measure,  /* <I> The measure type of the number */
    double *data_converted /* <O> The number converted to degree-based units */
);
...
(
    UF_KF_value_p_t   value, /* <I> Knowledge Fusion value */
    char            **string /* <OF> Value. Free with UF_free. */
);
...
(
    ...
    char **generated_name          /* <OF> name generated for the child object.
                                           Currently always returns NULL. Free with UF_free. */
);
...
/****************************************************************************
Ask the contents of an item in a list object.

If the dimension does not involve angles, then the numerical value
will always be in the base units of the part.

If the dimension involves angles, such as UF_MEASURE_ANGULAR_VELOCITY, then
the following rules apply:
    If the UF_KF_value holds a dimension, then the numerical value is radian-based.
        For angular velocity, then the numerical value would be in radians/s.
    If the UF_KF_value is dimensionless, then the numerical value is degree-based.
        For angular velocity, then the numerical value would be in degrees/s.

For numbers:
The numerical value is returned as a string according to the above rules, independent of the display units.
If the UF_KF_value does not hold a dimension or display unit, then the returned unit is nullptr.
The 'measure' argument may be nullptr, in which case no measure is returned.
...
Environment: Internal and External
See Also:
History: Originally release in v1872.0
***************************************************************************/
extern UFUNEXPORT int UF_KF_ask_list_item_value
(
    ...
    char           **datatype,    /* <OF> Data type of item.
                                          Free with UF_free */
    char           **unit,        /* <OF> Display unit type of item for number, if present, otherwise nullptr.
                                          Used only for list items, otherwise ignored.
                                          Free with UF_free. */
    char           **measure,     /* <OF> The unit measure for number, if dimensioned.
                                          If not dimensioned, will return UF_MEASURE_UNITLESS.
                                          Free with UF_free. */
...
NOTE: Deprecated.
      This function does not return the dimension or display units of the item.
...
This function will return the value of UF_KF_value object as well
as its dimension. The numerical value is returned degree-based for
angle-related dimensions, otherwise in base units.
...
(
    ...
    int              list_index,  /* <I>  Index of item in the list.
                                          (1 is the  first item) */
    char           **value_str,   /* <OF> Text of item's values.
                                          Free with UF_free. */
    char           **datatype,    /* <OF> Data type of item.
                                          Free with UF_free */
    UF_KF_value_p_t *sub_list_obj /* <OF> If this item is a list value, then
