...
::
call :Reg_Query "Unigraphics V31.0" "Unigraphics Solutions\Installed Applications"
::
...
::
  ...
  set VS_Tools_Path=%VS150COMNTOOLS%
  if not defined VS_Tools_Path (
  Rem Got nothing for %VAL. That means we couldn't find a registry key.
  echo. VS150COMNTOOLS environment variable is not set with Visual Studio %VC_VER% installed path or Visual Studio %VC_VER% is not installed. Visual Studio %VC_VER% install path was not found.
  set RETURN_CODE=2
  goto :END_SCRIPT
)
