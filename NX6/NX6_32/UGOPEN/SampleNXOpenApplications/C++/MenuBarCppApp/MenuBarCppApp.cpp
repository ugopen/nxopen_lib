...
//
//   Please see the MenuScript chapter of the Consoldated Programmer's Guide
...
//     - the NXOpen API Reference Manual
//     - the NXOpen API Programmer's Manual
//
//   *** PLEASE NOTE ***
...
MenuBar::MenuBarManager::CallbackStatus CppMenuBarApplication::PrintButtonIdCB( NXOpen::MenuBar::MenuButtonEvent* buttonEvent )
{
    if( !UF_initialize() )
    {
        ...
        sprintf( name, "%s", buttonEvent->ActiveButton()->ButtonName().GetLocaleText() );
        UF_UI_lock_ug_access( UF_UI_FROM_CUSTOM );
        response = uc1600( "Enter name of button you wish to query", name, &len );
        UF_UI_unlock_ug_access( UF_UI_FROM_CUSTOM );

        if ( response == 3 || response == 5 )
        {
            // get the button with this name
            MenuBar::MenuButton* myButton = theUI->MenuBarManager()->GetButtonFromName(name);
            if( myButton != NULL )
            {
                sprintf( msg, "    Button Id: %i", myButton->ButtonId() );
                lw->WriteLine( msg );
            }
            else
            {
		...
		{
                sprintf( msg, "    response: %i", response );
                lw->WriteLine( msg );
		}
		lw->WriteLine(" ");
    }
...
MenuBar::MenuBarManager::CallbackStatus CppMenuBarApplication::TestCallbackReturnsCB( NXOpen::MenuBar::MenuButtonEvent* buttonEvent )
{
    ...
    if( !UF_initialize() )
    {
        ...
        // set the default button name, to the name of the button which activated this event
        sprintf( name, "%s", buttonEvent->ActiveButton()->ButtonTypeName().GetLocaleText() );
        UF_UI_lock_ug_access( UF_UI_FROM_CUSTOM );
...
MenuBar::MenuBarManager::CallbackStatus CppMenuBarApplication::PrintApplicationIdCB( NXOpen::MenuBar::MenuButtonEvent* buttonEvent )
{
    ...
    sprintf( msg, "    Application Id: %i", buttonEvent->ApplicationId() );
    lw->WriteLine( msg );
    lw->WriteLine(" ");
    return MenuBar::MenuBarManager::CallbackStatusContinue;
}
...
MenuBar::MenuBarManager::CallbackStatus CppMenuBarApplication::PrintButtonDataCB( NXOpen::MenuBar::MenuButtonEvent* buttonEvent )
{
    ...
    sprintf( msg, "    Button Id: %i", buttonEvent->ActiveButton()->ButtonId() );
    lw->WriteLine( msg );
    sprintf( msg, "    Name: %s", buttonEvent->ActiveButton()->ButtonName().GetLocaleText() );
    lw->WriteLine( msg );
    sprintf( msg, "    Button type name: %s", buttonEvent->ActiveButton()->ButtonTypeName().GetLocaleText() );
    lw->WriteLine( msg );
    sprintf( msg, "    Menubar Name: %s", buttonEvent->MenuBarName().GetLocaleText() );
    lw->WriteLine( msg );
    sprintf( msg, "    Application Id: %i", buttonEvent->ApplicationId() );
    lw->WriteLine( msg );
    ...
    {
        sprintf( msg, "    Ancestor[%i]: %s", i, ancestors[i].GetLocaleText() );
        lw->WriteLine(msg);
    }
    lw->WriteLine(" ");
    return MenuBar::MenuBarManager::CallbackStatusContinue;
