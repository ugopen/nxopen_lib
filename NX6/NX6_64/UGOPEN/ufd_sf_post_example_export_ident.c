...
**
** Example name:
** ufd_sf_post_example_export_ident.c
**
** Description:
** An example demonstrating the use of the export & ident UGOPEN calls in
** UF_SF_POST. This example requires a SIM file with a result.
** Functions included here are:
**  1. UF_SF_POST_export_vrml
**  2. UF_SF_POST_export_display_to_text
**  3. UF_SF_POST_initialise_ident_params
**  4. UF_SF_POST_write_ident_data
**
** Work Flow:
**  1. Initialize Post processor
**  2. Export current results to vrml
**  3. Export current results to text
**  4. Change the required ident param members
**  5. Write ident data
...
static void do_ugopen_api(void)
{
    ...
    UF_SF_POST_ident_parms_t *ident_params = NULL;



    /* ufd_sf_post_example_export_ident starts here */
    sprintf(buffer_g, "Executing ufd_sf_post_example_export_ident.c ...\n");
...
/**/
    ...
    UF_CALL(UF_SF_POST_export_vrml("display2vrml.wrl"));

/**/

    /* Export display to text test */
/**/

    UF_CALL(UF_SF_POST_export_display_to_text("display2text.wrl", UF_SF_POST_FILE_VRML));
...
/**/
    ...
    /* Initialise the ident params test */
    action = UF_SF_POST_IDENT_N_HIGH_NODES;
    /* Store either number or data string. Here we store the number */
    number = 5;
    List_all_Comp_sw = true;
/**/

    UF_CALL(UF_SF_POST_initialise_ident_params(&ident_params, action, number, NULL, List_all_Comp_sw));

/**/

    /* Write the ident params test */
/**/

    UF_CALL( UF_SF_POST_write_ident_data(ident_params, NULL));
