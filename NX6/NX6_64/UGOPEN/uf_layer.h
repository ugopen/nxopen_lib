...
Always use the function UF_LAYER_ask_category_tag to get the tag_t of a category 
corresponding to name.The following could be such a function that would check it 
a category is created or not.
...
#define UF_LAYER_CAT_DESC_LEN        80
...
struct UF_LAYER_category_info_s {
    char name[MAX_ENTITY_NAME_SIZE+1];      /* The name of this category. */
    logical layer_mask[UF_LAYER_MAX_LAYER]; /* A logical for each layer, does
                                            ...
                                            */
    char descr[UF_LAYER_CAT_DESC_LEN+1];  /* This is the description of this
                                             layer category. */
};
...
extern UFUNEXPORT int uc5007(int *ip1, char *cp2, int ip3);

/* uc5008 read category layer -- replaced by UF_LAYER_ask_category_info */
extern UFUNEXPORT int uc5008(char *cp1, int ip2, int *ir3);

/* uc5009 edit layer category -- replaced UF_LAYER_edit_category_layer */
extern UFUNEXPORT int uc5009(int *ip1, char *cp2, int ip3);
