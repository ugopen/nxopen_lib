...
#region " UI Styler Dialog Designer generator code " 
    ...
    public int sel_cb(NXObject[] selectedObjects, NXObject[] deselectedObjects, SelectionHandle selectHandle)
    {
        try
        {
            for(int i =0; i < selectedObjects.Length; i++)
            {
                htEdges.Add(selectedObjects[i].JournalIdentifier, selectedObjects[i]);
            }
            for(int i =0; i < deselectedObjects.Length; i++)
            {
                htEdges.Remove(deselectedObjects[i].JournalIdentifier);
            }
