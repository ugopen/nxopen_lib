...
)
{
    ...
    uf5552(&note_tag, note_position, &error );
    if (error != 1)
    {
         fprintf(stderr,"Could not find position for note \n ");
    }
    error = UF_DISP_display_temporary_point(NULL_TAG, UF_DISP_USE_ACTIVE_PLUS, note_position, &color, UF_DISP_POUNDSIGN);
    if (error != 0)
    {
        fprintf(stderr, "Could not attention point note \n ");
    }
}
