...
*     
     ...
     =========================
           ...
           ufx_batchmeshing  -?  -h or -H 
                             ...
                             -c=<Criteria file name> -solver=<Solver name> -no_idealize  
            ...
            -solver=<Solver name>           : Solver name values are
                                                  ...
                                                  "ANSYS" 
                                                  "Abaqus" 
                                                  "Simcenter Thermal_OR_Flow"
                                                  "NX_Nastran Design"
                                                  "Simcenter Electronic Systems Cooling"
                                                  "Simcenter Space Systems Thermal"
                                                  "LS-DYNA"
                                                  "SC03"
                                                  "Simcenter MULTIPHYSICS"
                                                  "Ideas_Unv"
                                                  "MODAL_TEST_DATA"
                                                  "Samcef"
      ...
      =========================
         ...
         are the name strings build:
            ...
            <extn> values for language settings:
                ...
                NX_SPACE_SYSTEMS_THERMAL : ".xml"
                NX_ELECTRONIC_SYSTEMS_COOLING : ".xml"
                LSDYNA : ".k"
                IDEAS_UNV : ".unv"
                MODAL_TEST_DATA : ".unv"
                Samcef : ".dat"
...
static const char *scHelpText[] = {
    ...
    "                    -c=<Criteria file name> -t=<Template file name> -no_idealize",  
    "                    -fem -dat -solver=<Solver name> -jt -key_word=<some keyword string>",
    "             ",
    "             ",
    "   -?  -h or -H               : Help on usage.",
    "             ",
...
/* ==========================================
                  Defines
   ========================================== */
#define UF_UG_BATCH_MESH__VERSION         "2.0"  // Batch meshing version file
...
typedef struct UF_UG_BatchMesh_CommandLineOpts_s
{
    ...
    char            param_file_name[MAX_FSPEC_BUFSIZE];  /* FALSE=> Parm file not supplied */       
   
    int             fCriteriaFile;                                      /* TRUE => Criteria file specified */
    char            criteria_file_name[MAX_FSPEC_BUFSIZE]; /* FALSE=> Parm file not supplied */  
...
{
    char   scElemType[MAX_FSPEC_BUFSIZE];  /* Element type string 
                                                           ALL_MESH_TYPES : be smart to mesh
                                                                            sheets tris, solids test
                                                           TET_4, TET_10
                                                           TRI_3, TRI_6
                                                           QUAD_4, QUAD_8 */   
    char   mesh_collector[MAX_FSPEC_BUFSIZE]; /* Name of the Mesh Collector */
    int   param_body_type;                              /* Body Enums 
                                                           ALL_Body_TYPES : Do ALL VISIBLE BODIES 
                                                           SOLID, SHEET,SHEET_MS*/
    ...
    double    internal_mesh_gradation;       /* Internal Mesh Gradation */
    int       internal_max_edge_length_toggle;  /* Internal Max Edge Length Value Toggle */
    double    internal_max_edge_length_value;   /* Internak Max Edge Lenght Value */
    int       attempt_mapped_meshing;        /* Toggle for attempt mapped meshing */
    int       two_element_through_thickness_toggle;/* Toggle for two element through thickness */
    int       auto_fix_failed_elements_toggle;/* Toggle for auto fix failed elements */
    int       edge_merge_toggle;            /* Toggle for merge edge */
    double    edge_merge_angle;             /* Angle for merging edges */

    int       geometry_tolerance_toggle;     /* Toggle for Geometry Tolerance */
    double    geometry_tolerance_value ;     /* Geometry Tolerance */

    int       target_max_elem_length_toggle; /* Toggle for Target Maximum Element Length */
    double    target_max_elem_length_value;  /* Target Maximum Element Length */

    int       max_skew_toggle;               /* Toggle for Max Skew */
    double    max_skew;                      /* Max Skew */


    /* =============================
       Additional params for 2d mesh
       ============================= */
    int       edge_match_toggle;             /* Toggle for match edge */ 
    double    edge_match_tolerance;          /* UG edge matching
                                                tolerance. */ 
    /*Suppress Hole */
    int       suppress_hole_toggle;            /* Toggle for Suppress hole */
    double    suppress_hole_diameter_value;    /* Diameter for Suppress hole */
    int       suppress_hole_point_type;            /* Toggle for Suppress hole */

    int       multi_block_decomposition;          /* Toggle for Multi Block Decomposition */
    int       CAD_curvature_abstraction;          /* Toggle for CAD Curvature Abstraction */
    double    minimum_feature_length;             /* Toggle for Minimum Feature Length */
    int       target_minimum_element_edge_length; /* Toggle for Target Minimum Element Edge Length */
    ...
    int       max_warp_toggle;                     /* Max Warp Toggle*/
...
#define   UF_UG__BatchMesh_MESHParams(pzParams)     \
    ...
    strcpy((pzParams)->mesh_collector, ""); \
    (pzParams)->param_body_type = ALL_BODY_TYPES; \
    ...
    (pzParams)->attempt_mapped_meshing = true; \
    (pzParams)->multi_block_decomposition = FALSE;  \
    (pzParams)->two_element_through_thickness_toggle = false;     \
    (pzParams)->auto_fix_failed_elements_toggle = false;     \
    (pzParams)->geometry_tolerance_toggle = false;     \
    (pzParams)->geometry_tolerance_value = 0.01;     \
    (pzParams)->edge_merge_toggle = false;            \
    (pzParams)->edge_merge_angle = 15; \
    (pzParams)->edge_match_toggle = false;            \
    (pzParams)->edge_match_tolerance = 0.508; \
    (pzParams)->suppress_hole_toggle = false;  \
    (pzParams)->target_minimum_element_edge_length = FALSE; \
    (pzParams)->suppress_hole_diameter_value = 0.0; \
    (pzParams)->suppress_hole_point_type = 0; \
    (pzParams)->format_mesh = TRUE; \
    (pzParams)->attempt_quad_mapping = TRUE; \
    (pzParams)->quad_only_option = 0; \
    (pzParams)->split_poor_quads = FALSE; \
    (pzParams)->maximum_quad_warp = 10.0; \
    (pzParams)->max_warp_toggle = FALSE; \
    (pzParams)->move_nodes_off_geometry = FALSE; \
    (pzParams)->mesh_method = 0; \
    (pzParams)->max_included_angle_quad_toggle = false; \
    (pzParams)->min_included_angle_quad_toggle = false; \
    (pzParams)->max_included_angle_tria_toggle = false; \
    (pzParams)->min_included_angle_tria_toggle = false; \
    (pzParams)->max_included_angle_quad_value = 150.0; \
    (pzParams)->min_included_angle_quad_value = 30.0; \
    (pzParams)->max_included_angle_tria_value = 140.0; \
    (pzParams)->min_included_angle_tria_value = 20.0; \
    (pzParams)->target_max_elem_length_toggle = false; \
    (pzParams)->target_max_elem_length_value = 1.0; \
    (pzParams)->max_skew_toggle = false; \
    (pzParams)->max_skew = 30.0; \
    (pzParams)->internal_mesh_gradation = 1.05; \
    (pzParams)->internal_max_edge_length_toggle = false; \
    (pzParams)->internal_max_edge_length_value = 0.0; 
...
#define UF_UG__BatchMesh_GetDefaultSolverDeckFileExtn(scSolverName, scDeckExtn) \
          ...
          else if (!strcmp((scSolverName), "MSC_Nastran"))              \
             strcpy((scDeckExtn), ".dat");                              \
          ...
          else if (!strcmp((scSolverName), "Simcenter Thermal_OR_Flow"))\
             strcpy((scDeckExtn), ".xml");                              \
          else if (!strcmp((scSolverName), "NX_SPACE_SYSTEMS_THERMAL")) \
             strcpy ((scDeckExtn), ".xml");                             \
          else if (!strcmp((scSolverName), "Simcenter Space Systems Thermal")) \
             strcpy((scDeckExtn), ".xml");                              \
          else if (!strcmp((scSolverName), "NX_ELECTRONIC_SYSTEMS_COOLING"))       \
             strcpy ((scDeckExtn), ".xml");                              \
          else if (!strcmp((scSolverName), "Simcenter Electronic Systems Cooling")) \
             strcpy((scDeckExtn), ".xml");                               \
          else if (!strcmp((scSolverName), "LSDYNA"))                    \
             strcpy ((scDeckExtn), ".k");                                \
          else if (!strcmp((scSolverName), "LS - DYNA"))                 \
             strcpy((scDeckExtn), ".k");                                 \
          else if (!strcmp((scSolverName), "IDEAS_UNV") )                \
             strcpy ((scDeckExtn), ".unv");                             \
          else if (!strcmp((scSolverName), "Ideas_Unv"))                \
             strcpy((scDeckExtn), ".unv");                              \
          else if (!strcmp((scSolverName), "MODAL_TEST_DATA"))          \
             strcpy ((scDeckExtn), ".unv");                             \
          else if (!strcmp((scSolverName), "SAMCEF"))                   \
             strcpy ((scDeckExtn), ".dat");                             \
          else if (!strcmp((scSolverName), "Samcef"))                   \
             strcpy ((scDeckExtn), ".dat");                             \
          else                                                          \
...
if (!strcmp((scSolverName), "MSC_NASTRAN") || !strcmp((scSolverName), "MSC_Nastran"))                   \
             strcpy ((scDeckName), "MSC_NASTRAN");                             \
          else if (!strcmp((scSolverName), "NX_NASTRAN") || !strcmp((scSolverName), "NX_Nastran"))               \
          ...
          else if (!strcmp((scSolverName), "NX_THERMAL_OR_FLOW") || !strcmp((scSolverName), "Simcenter Thermal_OR_Flow"))       \
             strcpy ((scDeckName), "NX_THERMAL_OR_FLOW");                             \
          else if (!strcmp((scSolverName), "NX_SPACE_SYSTEMS_THERMAL") || !strcmp((scSolverName), "Simcenter Space Systems Thermal"))       \
             strcpy ((scDeckName), "NX_SPACE_SYSTEMS_THERMAL");                             \
          else if (!strcmp((scSolverName), "NX_ELECTRONIC_SYSTEMS_COOLING") || !strcmp((scSolverName), "Simcenter Electronic Systems Cooling"))       \
             strcpy ((scDeckName), "NX_ELECTRONIC_SYSTEMS_COOLING");                             \
          else if (!strcmp((scSolverName), "LSDYNA") || !strcmp((scSolverName), "LS-DYNA"))       \
             strcpy ((scDeckName), "LSDYNA");                             \
          else if (!strcmp((scSolverName), "IDEAS_UNV") || !strcmp((scSolverName), "Ideas_Unv"))       \
             strcpy ((scDeckName), "IDEAS_UNV");                             \
          else if (!strcmp((scSolverName), "MODAL_TEST_DATA"))       \
             strcpy ((scDeckName), "MODAL_TEST_DATA");               \
          else if (!strcmp((scSolverName), "Samcef") || !strcmp((scSolverName), "SAMCEF"))       \
...
/* =======================================
Check if mesh collector type set is 3D.
======================================= */
#define UF_UG__BatchMesh_Is3DMeshCollector(scMeshCollectorType)      \
          ( (!strcmp((scMeshCollectorType), "Solid")) ||    \
            (!strcmp((scMeshCollectorType), "Plotel Collector 3D")) ||    \
            (!strcmp((scMeshCollectorType), "Cohesive Collector"))     \
          )

/* =======================================
Check if mesh collector type set is 2D.
======================================= */
#define UF_UG__BatchMesh_Is2DMeshCollector(scMeshCollectorType)      \
          ( (!strcmp((scMeshCollectorType), "ThinShell")) ||    \
            (!strcmp((scMeshCollectorType), "Cshear Collector")) ||    \
            (!strcmp((scMeshCollectorType), "Plotel Collector 2D"))     \
          )
...
//static int open_part( const char *PartName, tag_t *part_tag);
...
static int create_sim__with_name_from_part_name ( tag_t         cad_part_tag,
                                                  ...
                                                  UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams,
                                                  tag_t         *new_sim_tag);
static int create_jt__with_name_from_part_name ( tag_t         tag_part_to_mesh,                                                  
                                                  UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams);
...
static void export_nastran_file ( tag_t cad_part_tag,
                                  tag_t fem_tag, const char* scSolverName, 
                                  UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams);
#if 0
...
static void get_solver_and_template(UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams, int partunits, char* scSolverName, char* scAnalysisType, char* scSolverTemplate);
static void log_pre_processing_info(int nCAEBodies,tag_t tag_part_to_mesh,
                               UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams);
static int log_processing_info( int index, tag_t iThis_Body_tag,
                                char **scElementName,
                                char      **scMeshType,
                                UF_UG_BatchMesh_Parms_t zMesh_params,
                                UF_SF_ugs_tet_mesh_parms_p_t pzmesh_3d_params,
                                UF_SF_ugs_2d_mesh_parms_p_t pzmesh_2d_params );
static void create_tet_mesh_on_body( tag_t iThis_Body_tag, char *scElementName, char* scMeshType, UF_UG_BatchMesh_Parms_t zMesh_params, UF_SF_ugs_tet_mesh_parms_t  mesh_3d_params, tag_t* mesh_tag);
static void create_2d_mesh_on_body( tag_t iThis_Body_tag, char *scElementName, char* scMeshType, UF_UG_BatchMesh_Parms_t zMesh_params, UF_SF_ugs_2d_mesh_parms_t  mesh_2d_params, tag_t* mesh_tag );

static int write_mesh_quality_report ( char                              *scSolverName,
                                       char                              *scMestType,
                                       tag_t                             mesh_tag,
                                       UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams,
                                       UF_UG_BatchMesh_Parms_t zMesh_params);
    ...
    if ( pzBatchRunParams->fOutputDAT==(int)true )
    {
        ...
        if ( iRc==0 &&  new_sim_tag!=NULL_TAG )
        {
            ...
            if (strlen(scPartDirectoryPath))
            {
                ...
                if (strcmp(scSolverName,"SAMCEF") == 0)
                {
                ...
                simPart1->ExportSimulation(scSolverName, caeDataContainer1);
...
static int do_Batch_Meshing__main ( UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams )
{
    ...
    try
    {
        ...
        char                           scCommandLineSolverName[MAX_LINE_BUFSIZE] = {' '};
        char                           scSolverTemplate[MAX_LINE_BUFSIZE] = {' '};
        char                           scCommandLineAnalysisType[MAX_LINE_BUFSIZE] = {' '};
        ...
        //Intialize meshing parameters 
        UF_UG_BatchMesh_Parms_t                    zMesh_params;
        UF_SF_ugs_tet_mesh_parms_t                 mesh_3d_params;
        UF_SF_ugs_2d_mesh_parms_t                  mesh_2d_params;    

        UF_SF_ugs_tet_mesh_parms_INIT ( &mesh_3d_params );
        UF_SF_ugs_2d_mesh_parms_INIT ( &mesh_2d_params );  

        //Intialize mesh control Geometry assignment 
        geomAssignment                    MCGeomAssignmentModule;
        ...
        basePart1 = theSession->Parts()->OpenBaseDisplay(pzBatchRunParams->part_file_name, &partLoadStatus1);
        tag_part_to_mesh = basePart1->Tag();
        
        Part *part(theSession->Parts()->Work());
        Part *workPart(theSession->Parts()->Work());
        Part *displayPart(theSession->Parts()->Display());
        NXOpen::View *workView = workPart->ModelingViews()->WorkView();
        std::vector<DisplayableObject *> displayedBodies(workView->AskVisibleObjects()); 
        delete partLoadStatus1;

        CAE::FemPart *workFemPart = NULL;
        CAE::FemPart *displayFemPart = NULL;
        ...
        //Solver and template based on the user input in command line argument
        get_solver_and_template ( pzBatchRunParams, partunits, scCommandLineSolverName, scCommandLineAnalysisType, scSolverTemplate );
       
        scSolverName = (NXString)scCommandLineSolverName;
        scAnalysisType = (NXString)scCommandLineAnalysisType;
        ...
        iRc = create_fem_name_from_part_name ( tag_part_to_mesh, scCommandLineSolverName,
                                               scCommandLineAnalysisType, 
                                               pzBatchRunParams->fOutIdealized,
                                               pzBatchRunParams,
                                               &Part_part_name,
                                               &Fem_part_name);

        //If template file is provided, batchmesher idetfies where this file is located and open is it up.
        // if there no template file, a new file file is created either based on command line argument or default template 
        ...
        { 
            try
            {
                basePart2 = theSession->Parts()->OpenBaseDisplay( pzBatchRunParams->template_file_name, &partLoadStatus2);
            }
            catch (const NXException &error)
            {                
                try
                {
                    if (isUnixEnv)
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"//" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                    else
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"\\" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                }
                catch (const NXException &error)
                {
                    ...
                    {
                        if (isUnixEnv)
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//simulation//templates//" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                        else
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\simulation\\templates\\" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                    }
                    catch (const NXException &error)
                    {
        ...
        else
        {
            ...
            {
                if (isUnixEnv)
                    basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"//" + scSolverTemplate), &partLoadStatus2);
                else
                    basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"\\" + scSolverTemplate), &partLoadStatus2);
            }
            catch (const NXException &error)
            {
                ...
                {                    
                    if (isUnixEnv)
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//simulation//templates//" + scSolverTemplate), &partLoadStatus2);
                    else
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\simulation\\templates\\" + scSolverTemplate), &partLoadStatus2);
                }
                catch (const NXException &error)
                {
                    if ( error.ErrorCode() != JA_ERR_UNEXPECTED_SUCCESS )
                    {
                        write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\nThe default template folder has not been set properly or default template cannot be found.\n");
                        return 9999;        
                    }
                }
            }  
        ...
        if (pzBatchRunParams->fOutputFEM==(int)true)
        {
            ...
            femPart1->AssignPermanentName(Fem_part_name);
            partSaveStatus2 = femPart1->Save(BasePart::SaveComponentsTrue, BasePart::CloseAfterSaveFalse);;   
            delete partSaveStatus2;  
        }

        if ( pzBatchRunParams->fTemplateFile==(int)true )
            femPart1->GetSolverAndAnalysisType( &scSolverName, &scAnalysisType );

        if ( !strcmp(scSolverName.GetLocaleText(), scCommandLineSolverName ))
        {
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, 
                "\nCommand line input solver language is overwritten by solver language in the provided template. \n" );
        }
        ...
        BodyCollection *theBodies = part->Bodies();
        ...
        if ( nCAEBodies <= 0 )
        {
            report_msg("Error : No visible bodies are found in the FEM.\n");
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "Error : No visible bodies to mesh. \n" );
            return 9999;
        }
        ...
        femCreationOptions1->SetCadData(part, Part_part_name);
        ...
        tag_t partTag = workFemPart->Tag();

        iRbody = UF_SF_ask_all_polygon_bodies (  &nCAEBodies, &CAEBodies_p );

        /* Get some material tag (Any one!) */
        iRMat = get_a_material ( &material_tag );     

        NXObject *partmaterialObj = (NXObject *)NXObjectManager::Get( material_tag );

        log_pre_processing_info( nCAEBodies,tag_part_to_mesh, pzBatchRunParams );

        // Geometry is assigned to Mesh controls if the template has been provide and in addition contains control definitions.
        MCGeomAssignmentModule.mesh_control_processing_module( UF_UG__BatchMesh_LOG, partTag, fEModel, nCAEBodies, CAEBodies_p );   
        ...
        for ( i=0; i<nCAEBodies; i++)
        {
            ...
            PhysicalMaterial *physicalMaterial1(dynamic_cast<PhysicalMaterial *>(partmaterialObj));
            std::vector<NXObject *> objects1(1);
            NXObject *partbodyObj = (NXObject *)NXObjectManager::Get( iThis_Body_tag );
            CAE::CAEBody *cAEBody1(dynamic_cast<CAE::CAEBody *>(partbodyObj));
            objects1[0] = cAEBody1;
            physicalMaterial1->AssignObjects(objects1);

            //All the Parameters are divided into either 2D Mesh or 3D Mesh
            iRbody = log_processing_info( i,iThis_Body_tag, &scElementName, &scMeshType, zMesh_params, &mesh_3d_params, &mesh_2d_params);
            ...
            if ( UF_UG__BatchMesh_Is3DMesh(scMeshType))
            {
                ...
                {
                    create_tet_mesh_on_body(iThis_Body_tag, scElementName, scMeshType, zMesh_params, mesh_3d_params, &mesh_tag);
                }
                else
                    iRc = -1;
            }
            ...
            {            
                /* ---- Mesh parameter settings : 2D mesh ---- */
                ...
                if ( iThis_Body_tag != NULL_TAG )
                {
                    create_2d_mesh_on_body(iThis_Body_tag, scElementName, scMeshType, zMesh_params, mesh_2d_params, &mesh_tag);
                }
                else
                    iRc = -1;
            }
            ...
            else
            {
                ...
                if (pzBatchRunParams->fOutputFEM==(int)true)
                {
                    ...
                    partSaveStatus1 = femPart1->Save(BasePart::SaveComponentsTrue, BasePart::CloseAfterSaveFalse);
                ...
                ================================================== */
                write_mesh_quality_report ( (char*)scSolverName.GetUTF8Text(), scMeshType, mesh_tag, pzBatchRunParams, zMesh_params);
            }
        ...
        {
            create_jt__with_name_from_part_name(tag_part_to_mesh,pzBatchRunParams );
        }
        ...
        ========================================== */
        if ( pzBatchRunParams->fOutputDAT==(int)true )
            export_nastran_file ( tag_part_to_mesh, new_fem_tag, scSolverName.GetUTF8Text(), pzBatchRunParams );
...
void create_2d_mesh_on_body( tag_t iThis_Body_tag, char *scElementName, char* scMeshType,                              
                             UF_UG_BatchMesh_Parms_t zMesh_params,
                             UF_SF_ugs_2d_mesh_parms_t mesh_2d_params,
                             tag_t* mesh_tag )
{
    ...
    CAE::FemPart *femPart1(dynamic_cast<CAE::FemPart *>(workFemPart));
    CAE::FEModel *fEModel1(dynamic_cast<CAE::FEModel *>(workFemPart->FindObject("FEModel")));
    CAE::MeshManager *meshManager(dynamic_cast<CAE::MeshManager *>(fEModel1->Find("MeshManager")));

    CAE::Mesh2d *nullCAE_Mesh2d(NULL);
    CAE::Mesh2dBuilder *mesh2dBuilder1;
    mesh2dBuilder1 = meshManager->CreateMesh2dBuilder(nullCAE_Mesh2d);
    tag_t partTag = workFemPart->Tag();
    CAE::MeshCollector *CAE_MeshCollector;

    std::vector<CAE::IMeshCollector *> MeshCollectors;
    std::vector<CAE::IMeshCollector *>::iterator iter;
    bool meshCollFound = false;
    MeshCollectors = meshManager->GetMeshCollectors();

    if (!MeshCollectors.empty())
    {
        for (iter = MeshCollectors.begin(); iter != MeshCollectors.end(); ++iter)
        {
            if ((strcmp(zMesh_params.mesh_collector, ((*iter)->Name()).GetText()) == 0) && UF_UG__BatchMesh_Is2DMeshCollector(((*iter)->CollectorNeutralType()).GetText()))
            {
                CAE::MeshCollector *tempCAE_MeshCollector(static_cast<CAE::MeshCollector *>(*iter));
                CAE_MeshCollector = tempCAE_MeshCollector;
                meshCollFound = true;
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "Information : Requested Mesh Collector: %s found in the template \n", zMesh_params.mesh_collector);
                break;
            }
        }
        if (!meshCollFound)
        {
            if (zMesh_params.mesh_collector[0] != '\0' && (strcmp(zMesh_params.mesh_collector, " ") != 0))
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "Information : Requested Mesh Collector: %s was not found in the template \n", zMesh_params.mesh_collector);
            }
            for (iter = MeshCollectors.begin(); iter != MeshCollectors.end(); ++iter)
            {
                if (UF_UG__BatchMesh_Is2DMeshCollector(((*iter)->CollectorNeutralType()).GetText()))
                {
                    write_info_fprintf(true, UF_UG__BatchMesh_LOG, "Information : Placing the mesh in the first available 2D mesh collector of the template\n");
                    CAE::MeshCollector *tempCAE_MeshCollector(static_cast<CAE::MeshCollector *>(*iter));
                    CAE_MeshCollector = tempCAE_MeshCollector;
                    break;
                }
    ...
    else
    {
        ...
        CAE_MeshCollector = mesh2dBuilder1->ElementType()->DestinationCollector()->ElementContainer();
    }

    mesh2dBuilder1->ElementType()->DestinationCollector()->SetElementContainer(CAE_MeshCollector);

    mesh2dBuilder1->SetGeometryUsageType(CAE::Mesh2dBuilder::GeometryTypeMain);

    mesh2dBuilder1->SetGeometryUsageType(CAE::Mesh2dBuilder::GeometryTypeMain);

    int iCurrentPartUnits = 0;
    UF_PART_ask_units( partTag, &iCurrentPartUnits );    
    
    int                        result, nCount, type, mainType, subType;
    tag_t                    meshcontrol;

    type = UF_caemisc_type;
    meshcontrol = NULL_TAG;
    nCount = 0;


    NXObject *bodyObj = (NXObject *)NXObjectManager::Get( iThis_Body_tag );                

    CAE::CAEBody *cAEBody1(dynamic_cast<CAE::CAEBody *>(bodyObj));

     
    bool added1;
    added1 = mesh2dBuilder1->SelectionList()->Add(cAEBody1);
    ...
    markId17 = theSession->SetUndoMark(Session::MarkVisibilityInvisible, "2D Mesh");

    theSession->DeleteUndoMark(markId17, NULL);

    Session::UndoMarkId markId18;
    markId18 = theSession->SetUndoMark(Session::MarkVisibilityInvisible, "2D Mesh");

    mesh2dBuilder1->ElementType()->SetElementDimension(CAE::ElementTypeBuilder::ElementTypeShell);

    mesh2dBuilder1->ElementType()->SetElementTypeName(scElementName);

    CAE::DestinationCollectorBuilder *destinationCollectorBuilder1;
    destinationCollectorBuilder1 = mesh2dBuilder1->ElementType()->DestinationCollector();

    destinationCollectorBuilder1->SetElementContainer(CAE_MeshCollector);

    destinationCollectorBuilder1->SetAutomaticMode(false);

    /*====================================================
                  Assign all the 2d parameters
    =====================================================*/
    mesh2dBuilder1->PropertyTable()->SetIntegerPropertyValue("meshing method", mesh_2d_params.mesh_method);

    Unit *unit1(dynamic_cast<Unit *>(workFemPart->UnitCollection()->FindObject("MilliMeter")));
    if (iCurrentPartUnits != UF_PART_METRIC)
            unit1 = (dynamic_cast<Unit *>(workFemPart->UnitCollection()->FindObject("Inch")));

    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("quad mesh overall edge size", mesh_2d_params.element_size , unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mapped mesh option bool", (mesh_2d_params.attempt_quad_mapping != 0));

    mesh2dBuilder1->PropertyTable()->SetIntegerPropertyValue("quad only option", mesh_2d_params.quad_only_option);

    mesh2dBuilder1->PropertyTable()->SetIntegerPropertyValue("midnodes", mesh_2d_params.midnode_option);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("geometry tolerance option bool", mesh_2d_params.geometry_tolerance_toggle);

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("geometry tolerance", mesh_2d_params.geometry_tolerance_toggle, unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("target maximum element edge length bool", zMesh_params.target_max_elem_length_toggle);

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("maximum element edge length", zMesh_params.target_max_elem_length_value, unit1);
    
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("target element skew bool", zMesh_params.max_skew_toggle);

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("element skew", zMesh_params.max_skew, unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("block decomposition option bool", mesh_2d_params.multi_block_decomposition);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("move nodes off geometry bool", zMesh_params.move_nodes_off_geometry);
    
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("max quad warp option bool", zMesh_params.max_warp_toggle);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("split poor quads bool", (mesh_2d_params.split_poor_quads != 0));

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("max included angle quad option bool", (mesh_2d_params.max_included_angle_quad_toggle != 0));

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("max included angle quad", (mesh_2d_params.max_included_angle_quad_value), unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("min included angle quad option bool", (mesh_2d_params.min_included_angle_quad_toggle != 0));

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("min included angle quad", (mesh_2d_params.min_included_angle_quad_value), unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("max included angle tria option bool", (mesh_2d_params.max_included_angle_tria_toggle != 0));

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("max included angle tria", (mesh_2d_params.max_included_angle_tria_value), unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("min included angle tria option bool", (mesh_2d_params.min_included_angle_tria_toggle != 0));

    mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("min included angle tria", (mesh_2d_params.min_included_angle_tria_value), unit1);


    Unit *nullUnit(NULL);
    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("max quad warp", mesh_2d_params.maximum_quad_warp, nullUnit);

    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("max jacobian", mesh_2d_params.maximum_midnode_jacobian, nullUnit);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh format to solver bool", (mesh_2d_params.format_mesh != 0));

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("target minimum element edge length", (mesh_2d_params.target_minimum_element_edge_length != 0));
    
    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("mesh size variation", mesh_2d_params.surf_mesh_size_variation, nullUnit);
        
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("alternate feature abstraction bool", (mesh_2d_params.CAD_curvature_abstraction != 0));
    
    if (mesh_2d_params.CAD_curvature_abstraction)
        mesh2dBuilder1->PropertyTable()->SetBaseScalarWithDataPropertyValue("minimum feature element size", mesh_2d_params.minimum_feature_length, unit1);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("quad mesh edge match toggle", (mesh_2d_params.edge_match_toggle != 0));

    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("quad mesh edge match tolerance", mesh_2d_params.edge_match_tolerance, unit1);

    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("small feature tolerance", mesh_2d_params.small_feature, nullUnit);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("merge edge toggle bool", (mesh_2d_params.edge_merge_toggle != 0));

    Unit *unit2(dynamic_cast<Unit *>(workFemPart->UnitCollection()->FindObject("Degrees")));
    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("edge angle", mesh_2d_params.edge_angle, unit2);

    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("min face angle", mesh_2d_params.minimum_face_angle, unit2);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("suppress hole option bool", (mesh_2d_params.suppress_hole_toggle != 0));
    
    mesh2dBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("hole diameter tolerance", mesh_2d_params.suppress_hole_diameter_value, unit1);
    
    mesh2dBuilder1->PropertyTable()->SetIntegerPropertyValue("hole suppresion point type", mesh_2d_params.suppress_hole_point_type);
    ...
    std::vector<CAE::Mesh *> meshes1;
    meshes1 = mesh2dBuilder1->CommitMesh();
    fEModel1->UpdateFemodel(); // Update FEM

    *mesh_tag = meshes1[0]->Tag();
...
void create_tet_mesh_on_body( tag_t iThis_Body_tag,char* scElementName, char* scMeshType, 
                              UF_UG_BatchMesh_Parms_t zMesh_params,   
                              UF_SF_ugs_tet_mesh_parms_t  mesh_3d_params,
                              tag_t* mesh_tag )
{
    ...
    CAE::Mesh3d *nullCAE_Mesh3d(NULL);
    CAE::Mesh3dTetBuilder *mesh3dTetBuilder1;
    mesh3dTetBuilder1 = meshManager->CreateMesh3dTetBuilder(nullCAE_Mesh3d);
    tag_t partTag = workFemPart->Tag();
    ...
    if (!MeshCollectors.empty())
    {
        for (iter = MeshCollectors.begin(); iter != MeshCollectors.end(); ++iter)
        {
            if((strcmp(zMesh_params.mesh_collector, ((*iter)->Name()).GetText()) == 0 ) && UF_UG__BatchMesh_Is3DMeshCollector(((*iter)->CollectorNeutralType()).GetText()))
            {
                CAE::MeshCollector *tempCAE_MeshCollector(static_cast<CAE::MeshCollector *>(*iter));
                CAE_MeshCollector = tempCAE_MeshCollector;
                meshCollFound = true;
        ...
        if (!meshCollFound)
        {
            ...
            for (iter = MeshCollectors.begin(); iter != MeshCollectors.end(); ++iter)
            {
                if (UF_UG__BatchMesh_Is3DMeshCollector(((*iter)->CollectorNeutralType()).GetText()))
                {
                    write_info_fprintf(true, UF_UG__BatchMesh_LOG, "Information : Placing the mesh in the first available 3D mesh collector of the template\n");
                    CAE::MeshCollector *tempCAE_MeshCollector(static_cast<CAE::MeshCollector *>(*iter));
                    CAE_MeshCollector = tempCAE_MeshCollector;
                    break;
                }
    ...
    else
    {
        ...
        CAE_MeshCollector = mesh3dTetBuilder1->ElementType()->DestinationCollector()->ElementContainer();
    }
    
    mesh3dTetBuilder1->ElementType()->DestinationCollector()->SetElementContainer(CAE_MeshCollector);
    ...
    mesh3dTetBuilder1->ElementType()->SetElementDimension(CAE::ElementTypeBuilder::ElementTypeFreeSolid);   

    CAE::DestinationCollectorBuilder *destinationCollectorBuilder1;
    destinationCollectorBuilder1 = mesh3dTetBuilder1->ElementType()->DestinationCollector();

    mesh3dTetBuilder1->ElementType()->SetElementTypeName(scElementName);
    ...
    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("quad mesh overall edge size", mesh_3d_params.element_size, unit1);

    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("mapped mesh option bool", (mesh_3d_params.attempt_mapped_meshing != 0));
    if ((mesh_3d_params.attempt_mapped_meshing != 0))
        mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("multiblock cylinder option bool", (mesh_3d_params.multi_block_decomposition != 0));

    mesh3dTetBuilder1->PropertyTable()->SetIntegerPropertyValue("midnodes", mesh_3d_params.midnode_option);

    Unit *nullUnit(NULL);
    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("max jacobian", mesh_3d_params.maximum_midnode_jacobian, nullUnit);

    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("surface mesh size variation", mesh_3d_params.surf_mesh_size_variation, nullUnit);

    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("two elements through thickness bool", (mesh_3d_params.two_element_through_thickness_toggle != 0));
    
    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("remesh on bad quality bool", (mesh_3d_params.auto_fix_failed_elements_toggle != 0) );

    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("small feature tolerance", mesh_3d_params.small_feature, nullUnit);

    Unit *unit2(dynamic_cast<Unit *>(workFemPart->UnitCollection()->FindObject("Degrees")));
    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("min face angle", mesh_3d_params.minimum_face_angle, unit2);

    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("edge angle", mesh_3d_params.edge_angle, unit2);

    mesh3dTetBuilder1->PropertyTable()->SetIntegerPropertyValue("merge edge toggle", mesh_3d_params.edge_merge_toggle);

    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("internal mesh gradation", zMesh_params.internal_mesh_gradation, nullUnit);

    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("internal max edge option bool", (zMesh_params.internal_max_edge_length_toggle != 0));

    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("internal max edge length value", zMesh_params.internal_max_edge_length_value, unit1);
    ...
    std::vector<CAE::Mesh *> meshes1;
    meshes1 = mesh3dTetBuilder1->CommitWithPartialMesh();                
...
int log_processing_info( int index, tag_t iThis_Body_tag,
                         char **scElementName,
                         char **scMeshType, 
                         UF_UG_BatchMesh_Parms_t zMesh_params,
                         UF_SF_ugs_tet_mesh_parms_p_t pzmesh_3d_params,
                         UF_SF_ugs_2d_mesh_parms_p_t pzmesh_2d_params)
{
    int iRc = 0,mem_error = 0 ;
    int body_type = 0;
    double                         dElemSize = 0.0;
    
    *scMeshType = (char* )UF_allocate_memory(MAX_FSPEC_BUFSIZE*sizeof(char), &mem_error);
    *scElementName = (char* )UF_allocate_memory(MAX_FSPEC_BUFSIZE*sizeof(char), &mem_error);
    ...
    /* Start processing message */
        ...
        write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "Body No : %d (Body tag : %u )\n",
                             index+1, iThis_Body_tag);

        if (body_type==0)
        {
            if ( body_type == zMesh_params.param_body_type || 
                zMesh_params.param_body_type == ALL_BODY_TYPES )
            ...
            {
                write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\tInvalid Param file Body type: Skip meshing this body.\n");
                return 1;
            }
        }
        else if (body_type==1)
        ...
        else if (body_type==2)
        {
            ...
            {
                /* Body type : SHEET BODY [MID-SURFACE] */
        ...
        /* === Set up mesh params === */
        pzmesh_3d_params->midnodes                   = zMesh_params.midnodes;
        pzmesh_3d_params->element_size               = zMesh_params.element_size;
        pzmesh_3d_params->midnode_option             = zMesh_params.midnode_option;
        pzmesh_3d_params->maximum_midnode_jacobian   = zMesh_params.maximum_midnode_jacobian;
        pzmesh_3d_params->minimum_face_angle         = zMesh_params.minimum_face_angle;
        pzmesh_3d_params->surf_mesh_size_variation   = zMesh_params.surf_mesh_size_variation;

        if ( zMesh_params.attempt_mapped_meshing==(int)true )        
            pzmesh_3d_params->attempt_mapped_meshing = (int)true;        
        else
            pzmesh_3d_params->attempt_mapped_meshing = (int)false;

        if ( zMesh_params.two_element_through_thickness_toggle==(int)true )        
            pzmesh_3d_params->two_element_through_thickness_toggle = (int)true;        
        else
            pzmesh_3d_params->two_element_through_thickness_toggle = (int)false;       

        if ( zMesh_params.auto_fix_failed_elements_toggle==(int)true )        
            pzmesh_3d_params->auto_fix_failed_elements_toggle = (int)true;        
        else
            pzmesh_3d_params->auto_fix_failed_elements_toggle = (int)false;

        if (zMesh_params.geometry_tolerance_toggle == (int)true)
        {
            pzmesh_2d_params->geometry_tolerance_toggle = (int)true;
            pzmesh_3d_params->geometry_tolerance_toggle = (int)true;
        }
        else
        {
            pzmesh_2d_params->geometry_tolerance_toggle = (int)false;
            pzmesh_3d_params->geometry_tolerance_toggle = (int)false;
        }

        if (zMesh_params.geometry_tolerance_value == (int)true)
        {
            pzmesh_2d_params->geometry_tolerance_value = (int)true;
            pzmesh_3d_params->geometry_tolerance_value = (int)true;
        }
        else
        {
            pzmesh_2d_params->geometry_tolerance_value = (int)false;
            pzmesh_3d_params->geometry_tolerance_value = (int)false;
        }
        

        pzmesh_2d_params->element_size               = zMesh_params.element_size;                                
        pzmesh_2d_params->midnode_option             = zMesh_params.midnode_option;
        pzmesh_2d_params->maximum_midnode_jacobian   = zMesh_params.maximum_midnode_jacobian;         
        pzmesh_2d_params->minimum_face_angle         = zMesh_params.minimum_face_angle;               
        pzmesh_2d_params->surf_mesh_size_variation   = zMesh_params.surf_mesh_size_variation;
        /* Some additional opts specific to 2d mesh */
        pzmesh_2d_params->suppress_hole_diameter_value = zMesh_params.suppress_hole_diameter_value;
        pzmesh_2d_params->suppress_hole_point_type     = zMesh_params.suppress_hole_point_type;
        pzmesh_2d_params->edge_match_tolerance       = zMesh_params.edge_match_tolerance;            
        pzmesh_2d_params->format_mesh                = zMesh_params.format_mesh;                        
        pzmesh_2d_params->attempt_quad_mapping       = zMesh_params.attempt_quad_mapping;     
        pzmesh_2d_params->quad_only_option           = zMesh_params.quad_only_option;    
        pzmesh_2d_params->split_poor_quads           = zMesh_params.split_poor_quads;                    
        pzmesh_2d_params->maximum_quad_warp          = zMesh_params.maximum_quad_warp;     
        pzmesh_2d_params->mesh_method                = zMesh_params.mesh_method;

        pzmesh_2d_params->max_included_angle_quad_toggle = zMesh_params.max_included_angle_quad_toggle;
        pzmesh_2d_params->min_included_angle_quad_toggle = zMesh_params.min_included_angle_quad_toggle;
        pzmesh_2d_params->max_included_angle_tria_toggle = zMesh_params.max_included_angle_tria_toggle;
        pzmesh_2d_params->min_included_angle_tria_toggle = zMesh_params.min_included_angle_tria_toggle;

        pzmesh_2d_params->max_included_angle_quad_value = zMesh_params.max_included_angle_quad_value;
        pzmesh_2d_params->min_included_angle_quad_value = zMesh_params.min_included_angle_quad_value;
        pzmesh_2d_params->max_included_angle_tria_value = zMesh_params.max_included_angle_tria_value;
        pzmesh_2d_params->min_included_angle_tria_value = zMesh_params.min_included_angle_tria_value;

        pzmesh_2d_params->move_nodes_off_geometry = zMesh_params.move_nodes_off_geometry;

        pzmesh_2d_params->max_warp_toggle = zMesh_params.max_warp_toggle;
        
        if ( zMesh_params.suppress_hole_toggle==(int)true )        
            pzmesh_2d_params->suppress_hole_toggle = (int)true;        
        else
            pzmesh_2d_params->suppress_hole_toggle = (int)false;         

        if ( zMesh_params.edge_match_toggle==(int)true )        
            pzmesh_2d_params->edge_match_toggle = (int)true;        
        else
            pzmesh_2d_params->edge_match_toggle = (int)false;         

        if ( zMesh_params.target_minimum_element_edge_length==(int)false )        
            pzmesh_2d_params->target_minimum_element_edge_length = (int)false;        
        else
            pzmesh_2d_params->target_minimum_element_edge_length = (int)true;

        if ( zMesh_params.CAD_curvature_abstraction==(int)false )        
            pzmesh_2d_params->CAD_curvature_abstraction = (int)false;        
        else
            pzmesh_2d_params->CAD_curvature_abstraction = (int)true;

        if (zMesh_params.multi_block_decomposition == (int)false)
        {
            pzmesh_2d_params->multi_block_decomposition = (int)false;
            pzmesh_3d_params->multi_block_decomposition = (int)false;
        }
        else
        {
            pzmesh_2d_params->multi_block_decomposition = (int)true;
            pzmesh_3d_params->multi_block_decomposition = (int)true;
        }

        if ( zMesh_params.CAD_curvature_abstraction==(int)true )
            pzmesh_2d_params->minimum_feature_length            = zMesh_params.minimum_feature_length;
        else
            pzmesh_2d_params->minimum_feature_length = 0;
       
        /* Abstraction parameters */
        pzmesh_3d_params->small_feature = zMesh_params.small_feature_size; 
        pzmesh_2d_params->small_feature = zMesh_params.small_feature_size; 
        pzmesh_3d_params->edge_angle = 10.0;

        /* Edge merge toggle 0 or 1 */
        if ( zMesh_params.edge_merge_toggle==(int)true )  
        {
            pzmesh_3d_params->edge_merge_toggle = (int)true;
            pzmesh_3d_params->edge_angle = zMesh_params.edge_merge_angle;

            pzmesh_2d_params->edge_merge_toggle = (int)true;
            pzmesh_2d_params->edge_angle = zMesh_params.edge_merge_angle;
        }
        else
        {
            pzmesh_3d_params->edge_merge_toggle = (int)false;

            pzmesh_2d_params->edge_merge_toggle = (int)false;
        }
        ...
        if ( zMesh_params.fAutoSize == (int)true )  
        {
            ...
            pzmesh_3d_params->element_size = dElemSize;
            pzmesh_2d_params->element_size = dElemSize;
        }
        ...
        if ( UF_UG__BatchMesh_Is2DMesh(zMesh_params.scElemType))
        {
            ...
            UF_SF_ask_element_type_names( element_type, &element_name);
            strcpy (*scElementName, element_name); 
            UF_free(element_name);
            ...
            if ( body_type==0 )
            {
                ...
                return 1 ;/* Contiunue to next body in list */
            }
        }
        else if ( UF_UG__BatchMesh_Is3DMesh(zMesh_params.scElemType) )
        {
            ...
            if ( !strcmp(zMesh_params.scElemType, "TET_10" ))
            {
                strcpy(*scMeshType, "TET_10");
                pzmesh_3d_params->element_type = UF_SF_TET10;
            }
            else
            {
                strcpy(*scMeshType, "TET_4");
                pzmesh_3d_params->element_type = UF_SF_TET4;
            }
            
            char* element_name = NULL;
            UF_SF_ask_element_type_names( pzmesh_3d_params->element_type, &element_name);
            strcpy (*scElementName, element_name); 
            UF_free(element_name);
        ...
        else 
        {
            ...
            if ( body_type!=0 )
            {
                ...
                write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\tBody Type : Sheet (Use default %s).\n",
                                     *scMeshType);
            }
            else 
            {
                /* Solid and mesh type set to default (i.e., not set)*/
                if (pzmesh_3d_params->midnodes == (int)true)
                {
                    strcpy(*scMeshType, "TET_10");
                    element_type = UF_SF_TET10;
                }
                else
                {
                    strcpy(*scMeshType, "TET_4");
                    element_type = UF_SF_TET4;
                }
        ...
        if (UF_UG__BatchMesh_Is2DMesh(zMesh_params.scElemType))
        {
            if (pzmesh_2d_params->CAD_curvature_abstraction == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  CAD Curvature Abstraction:  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t         Minimum Feature Length :  %f\n",
                    pzmesh_2d_params->minimum_feature_length);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  CAD Curvature Abstraction :  OFF\n");

            if (pzmesh_2d_params->suppress_hole_toggle == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  Suppress Hole Toggle :  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t         Hole Diameter :  %f\n",
                    pzmesh_2d_params->suppress_hole_diameter_value);
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t        Point Creation :  %f\n",
                    pzmesh_2d_params->suppress_hole_point_type);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  Suppress Hole Toggle :  OFF\n");


            if (pzmesh_2d_params->edge_match_toggle == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  Match Edges Toggle    :  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  Match Edges Tolerance :  %f\n",
                    pzmesh_2d_params->edge_match_tolerance);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t  Match Edges Toggle   :  OFF\n");

            if (pzmesh_2d_params->edge_merge_toggle == (int)true)
            {
        ...
        write_info_fprintf(true, UF_UG__BatchMesh_LOG, "\t   Small Feature Size = %f %% Element Size\n",
            pzmesh_2d_params->small_feature);
                               ...
                               UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams)
{
     ...
     if ( pzBatchRunParams->fOutputJT==(int)true)
     {
        ...
        build_name_srting ( 7, tag_part_to_mesh,pzBatchRunParams, &JT_MESH_file_Name);
...
void get_solver_and_template ( UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams, int partunits, char* scSolverName, char* scAnalysisType, char* scSolverTemplate )
{
    ...
    /* Solver name string */
     if ( !strcmp(pzBatchRunParams->Solver_Name, "Structures_P_E" ))
     {
         ...
         strcpy (scAnalysisType, "Structural");
               ...
               !strcmp(pzBatchRunParams->Solver_Name, "MSC_Nastran"))
     {
         strcpy (scSolverName , "MSC NASTRAN" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemMscNastranMetric.fem");
         else
             strcpy (scSolverTemplate, "FemMscNastranEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_NASTRAN" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "NX_Nastran"))
     {
         strcpy (scSolverName , "NX NASTRAN" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxNastranEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"ANSYS" ))
     {
         ...
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemAnsysMetric.fem");
         else
             strcpy (scSolverTemplate, "FemAnsysEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"ABAQUS" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Abaqus"))
     {
         strcpy (scSolverName , "ABAQUS"  );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemAbaqusMetric.fem");
         else
             strcpy (scSolverTemplate, "FemAbaqusEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_THERMAL_OR_FLOW" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Simcenter Thermal_OR_Flow"))
     {
         strcpy (scSolverName , "NX THERMAL / FLOW" );
         strcpy (scAnalysisType, "Thermal");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemThermalFlowMetric.fem");
         else
             strcpy (scSolverTemplate, "FemThermalFlowEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_NASTRAN_DESIGN" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "NX_Nastran_Design"))
     {
         strcpy (scSolverName , "NX NASTRAN DESIGN" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranDesignMetric.fem");
     else
             strcpy (scSolverTemplate, "FemNxNastranDesignEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_ELECTRONIC_SYSTEMS_COOLING" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Simcenter Electronic Systems Cooling"))
     {
         strcpy (scSolverName , "NX ELECTRONIC SYSTEMS COOLING" );
         strcpy (scAnalysisType, "Coupled Thermal-Flow");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemElecSysCoolingMetric.fem");
         else
             strcpy (scSolverTemplate, "FemElecSysCoolingEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_SPACE_SYSTEMS_THERMAL" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Simcenter Space Systems Thermal") )
     {
         strcpy (scSolverName , "NX SPACE SYSTEMS THERMAL" );
         strcpy (scAnalysisType, "Thermal");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemSpaceSysThermalMetric.fem");
         else
             strcpy (scSolverTemplate, "FemSpaceSysThermalEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"LSDYNA" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "LS-DYNA"))
     {
         strcpy (scSolverName , "LSDYNA" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemLSDynaMetric.fem");
         else
             strcpy (scSolverTemplate, "FemLSDynaEnglish.fem");
     }     
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"SC03" ))
     {
         strcpy (scSolverName , "SC03" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
         else
     ...
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_MULTIPHYSICS" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Simcenter Multiphysics"))
     {
         strcpy (scSolverName , "NX MULTIPHYSICS" );
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
         else
     ...
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"IDEAS_UNV" ) ||
               !strcmp(pzBatchRunParams->Solver_Name, "Ideas_Unv") )
     {
         strcpy (scSolverName , "IDEAS UNV" );
         strcpy (scAnalysisType, "Universal");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxNastranEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"MODAL_TEST_DATA" ))
     {
         strcpy (scSolverName , "MODAL TEST DATA" );
         strcpy (scAnalysisType, "Structural Modal Test");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxNastranEnglish.fem");
     }
     else if (!strcmp(pzBatchRunParams->Solver_Name, "Samcef") ||
         !strcmp(pzBatchRunParams->Solver_Name, "SAMCEF"))
     {
         strcpy(scSolverName, "SAMCEF");
         strcpy(scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy(scSolverTemplate, "FemSamcefMetric.fem");
         else
             strcpy(scSolverTemplate, "FemSamcefEnglish.fem");
     }
     else
     {
         /* Default is NX nastran */
         if ( !strcmp(pzBatchRunParams->Solver_Name,"--Not_Specified--" ) )
         {
             if (partunits == UF_PART_METRIC)
                 strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
             else
             ...
             strcpy (scAnalysisType, "Structural");
              write_info_fprintf ( true, UF_UG__BatchMesh_LOG, 
             "No solver language option specified : Default will be used if a template has not been provided.\n" );
             
         }
         else
         {
             strcpy (scSolverName , pzBatchRunParams->Solver_Name );
              write_info_fprintf ( true, UF_UG__BatchMesh_LOG, 
             "A New solver language option specified.\n" );
         }
         strcpy (scAnalysisType, "Structural");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxNastranMetric.fem");
...
static int get_a_material ( tag_t    *material_tag)
{
    ...
    char            *type_filter = "IsotropicMaterial";      /* Doesn't matter take any 
                                  ...
                                  UF_UG_BatchMesh_Parms_t           *pzMesh_params)
{
    ...
    char                           scMeshType[MAX_FSPEC_BUFSIZE]={' '};
    char                           scBodyType[MAX_FSPEC_BUFSIZE]={' '};
    char                           scMidNodeOpt[MAX_FSPEC_BUFSIZE]={' '};
    char                           sc2DMeshMethod[MAX_FSPEC_BUFSIZE]={' '};
    char                           scMeshCollector[MAX_FSPEC_BUFSIZE] = { ' ' };
    ...
    if ( pzBatchRunParams->fParamFile==(int)true)
    {
        ...
        if ( pzBatchRunParams->fParamFile==(int)true)
        {
            ...
            else
            {
                ...
                /* === mesh parameters === */
                meshParamReader.Get_body_type (scBodyType);
                if ( strcmp(scBodyType,"SOLID") == 0 )
                {
                    pzMesh_params->param_body_type = 0;
                }
                else if ( strcmp(scBodyType,"SHEET") == 0 )
                {
                    pzMesh_params->param_body_type = 1;
                }
                else if ( strcmp(scBodyType,"SHEET_MS") == 0 )
                {
                    pzMesh_params->param_body_type = 2;
                }
                else
                {
                    pzMesh_params->param_body_type = ALL_BODY_TYPES;
                }
                ...
                meshParamReader.Get_mesh_collector(scMeshCollector);
               
                if (isUnixEnv)
                {
                    strncpy(pzMesh_params->mesh_collector, scMeshCollector, strlen(scMeshCollector) - 1);
                    pzMesh_params->mesh_collector[strlen(scMeshCollector) - 1] = '\0';
                }
                else
                {
                    strcpy(pzMesh_params->mesh_collector, scMeshCollector);
                }
                ...
                meshParamReader.Get_internal_mesh_gradation(&pzMesh_params->internal_mesh_gradation);
                meshParamReader.Get_internal_max_edge_length_toggle(&pzMesh_params->internal_max_edge_length_toggle);
                meshParamReader.Get_internal_max_edge_length_value(&pzMesh_params->internal_max_edge_length_value);

                /* Mapped Mesh Toggle */
                meshParamReader.Get_attempt_mapped_meshing_ON_OFF (&pzMesh_params->attempt_mapped_meshing );

                meshParamReader.Get_two_element_through_thickness_ON_OFF (&pzMesh_params->two_element_through_thickness_toggle);

                meshParamReader.Get_auto_fix_failed_elements_ON_OFF (&pzMesh_params->auto_fix_failed_elements_toggle);

                meshParamReader.Get_geomtery_tolerance_toggle(&pzMesh_params->geometry_tolerance_toggle);

                meshParamReader.Get_geomtery_tolerance_toggle(&pzMesh_params->geometry_tolerance_toggle);

                /* Additional quad/2d mesh related parameters */
                meshParamReader.Get_match_edge_ON_OFF_Toggle (&pzMesh_params->edge_match_toggle);
                ...
                meshParamReader.Get_target_max_elem_length_toggle(&pzMesh_params->target_max_elem_length_toggle);
                meshParamReader.Get_target_max_elem_length_value(&pzMesh_params->target_max_elem_length_value);
                meshParamReader.Get_max_skew_toggle(&pzMesh_params->max_skew_toggle);
                meshParamReader.Get_max_skew_value(&pzMesh_params->max_skew);

                meshParamReader.Get_max_included_angle_quad_toggle(&pzMesh_params->max_included_angle_quad_toggle);
                meshParamReader.Get_min_included_angle_quad_toggle(&pzMesh_params->min_included_angle_quad_toggle);
                meshParamReader.Get_max_included_angle_tria_toggle(&pzMesh_params->max_included_angle_tria_toggle);
                meshParamReader.Get_min_included_angle_tria_toggle(&pzMesh_params->min_included_angle_tria_toggle);

                meshParamReader.Get_max_included_angle_quad_value(&pzMesh_params->max_included_angle_quad_value);
                meshParamReader.Get_min_included_angle_quad_value(&pzMesh_params->min_included_angle_quad_value);
                meshParamReader.Get_max_included_angle_tria_value(&pzMesh_params->max_included_angle_tria_value);
                meshParamReader.Get_min_included_angle_tria_value(&pzMesh_params->min_included_angle_tria_value);

                meshParamReader.Get_move_nodes_off_geometry(&pzMesh_params->move_nodes_off_geometry);
                meshParamReader.Get_max_warp_toggle_ON_OFF(&pzMesh_params->max_warp_toggle);
                ...
                meshParamReader.Get_target_min_elm_edge_length (&pzMesh_params->target_minimum_element_edge_length); 
                meshParamReader.Get_CAD_curvature_abstraction (&pzMesh_params->CAD_curvature_abstraction); 
                meshParamReader.Get_minimum_feature_length (&pzMesh_params->minimum_feature_length); 
                meshParamReader.Get_multi_block_decomposition(&pzMesh_params->multi_block_decomposition);
                ...
                /* === Suppress Hole params === */
                meshParamReader.Get_suppress_hole_ON_OFF_Toggle (&pzMesh_params->suppress_hole_toggle);
                
                char scPointType[MAX_FSPEC_BUFSIZE]={' '};
                if (pzMesh_params->suppress_hole_toggle==(int)true)
                {
                    meshParamReader.Get_suppress_hole_diameter_value (&pzMesh_params->suppress_hole_diameter_value);
                    meshParamReader.Get_suppress_hole_point_type (scPointType);
                    if ( strcmp(scPointType,"NONE") == 0 )
                    {
                        pzMesh_params->suppress_hole_point_type = 0;
                    }
                    else if ( strcmp(scPointType,"POINT") == 0 )
                    {
                        pzMesh_params->suppress_hole_point_type = 1;
                    }
                    else if ( strcmp(scPointType,"MESHPOINT") == 0 )
                    {
                        pzMesh_params->suppress_hole_point_type = 2;
                    }
                    else
                    {
                        pzMesh_params->suppress_hole_point_type = 0;
                ...
                {
                  pzMesh_params->suppress_hole_diameter_value = 0;
                  pzMesh_params->suppress_hole_point_type = 0;
                }
                                       ...
                                       UF_UG_BatchMesh_Parms_t            zMesh_params)
{
    ...
    int       max_included_angle_quad_tog = 0;       /* Max Angle for quad Toggle state */
    int       min_included_angle_quad_tog = 0;       /* Min Angle for quad Toggle state */
    int       max_included_angle_tria_tog = 0;       /* Max Angle for tria Toggle state */
    int       min_included_angle_tria_tog = 0;       /* Min Angle for tria Toggle state */
    double    max_included_angle_quad_val = 150.0;       /* Max Angle for quad value */
    double    min_included_angle_quad_val = 30.0;       /* Min Angle for quad value */
    double    max_included_angle_tria_val = 140.0;       /* Max Angle for tria value */
    double    min_included_angle_tria_val = 20.0;       /* Min Angle for tria value */
    ...
    if (zMesh_params.multi_block_decomposition == (int)true)
    {
        write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Multi Block Decomposition:  ON\n");
    }
    else
        write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Multi Block Decomposition :  OFF\n");
    ...
    if ( fUse_2D_params==TRUE )
    {
       ...
       /* write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Edge match Tol = %f\n", 
        ...
        if (zMesh_params.max_included_angle_quad_toggle == (int)true)  
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for quad Toggle = ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for quad = %f\n", 
                zMesh_params.max_included_angle_quad_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for quad Toggle = OFF\n");

        if (zMesh_params.min_included_angle_quad_toggle == (int)true)  
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for quad Toggle = ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for quad = %f\n",
                zMesh_params.min_included_angle_quad_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for quad Toggle = OFF\n");

        if (zMesh_params.max_included_angle_tria_toggle == (int)true) 
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria Toggle = ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria = %f\n",
                zMesh_params.max_included_angle_tria_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria Toggle = OFF\n");

        if (zMesh_params.min_included_angle_tria_toggle == (int)true) 
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for tria Toggle = ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for tria = %f\n",
                zMesh_params.min_included_angle_tria_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for tria Toggle = OFF\n");
        ...
        //{
            ...
            /* Quad warp param */
            /* Splitting quads */
            if (zMesh_params.max_warp_toggle == (int)true)
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Warp = ON\n");
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Warp = OFF\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max. QUAD warp = %f\n",
                               mesh_2d_parms.maximum_quad_warp);

            if (zMesh_params.geometry_tolerance_toggle == (int)true )
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Geometry Tolerance    :  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Geometry Tolerance Value    :  %f\n",
                    zMesh_params.geometry_tolerance_value);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Geometry Tolerance    :  Off\n");


            if (zMesh_params.target_minimum_element_edge_length == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Minimum Element Edge Length:  ON\n");
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Minimum Element Edge Length :  OFF\n");

            if (zMesh_params.target_max_elem_length_toggle == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Max Element Length Toggle :  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Max Element Length :  %f\n",
                    zMesh_params.target_max_elem_length_value);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Max Element Length Toggle :  OFF\n");

            if (zMesh_params.max_skew_toggle == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Skew Toggle :  ON\n");
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Skew :  %f\n",
                    zMesh_params.max_skew);
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Skew Toggle :  OFF\n");

            if (zMesh_params.move_nodes_off_geometry == (int)true)
            {
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Move Nodes Off Geometry:  ON\n");
            }
            else
                write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Move Nodes Off Geometry:  OFF\n");
    ...
    else
    {
        ...
        write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Internal Mesh Gradation = %f\n",
            zMesh_params.internal_mesh_gradation);

        if (zMesh_params.attempt_mapped_meshing == (int)true)
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Attempt Free Mapped Meshing    :  ON\n");
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Attempt Free Mapped Meshing    :  OFF\n");

        if (zMesh_params.two_element_through_thickness_toggle == (int)true)
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Minimum Two Elemnt Through Thickness    :  ON\n");
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Minimum Two Elemnt Through Thickness    :  OFF\n");

        if (zMesh_params.internal_max_edge_length_toggle == (int)true)
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Internal Edge Length Limit  :  ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Internal Edge Length Limit   :  %f\n",
                zMesh_params.internal_max_edge_length_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Target Internal Edge Length Limit   :  OFF\n");

        if (zMesh_params.auto_fix_failed_elements_toggle == (int)true)
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Auto Fix Failed Elements    :  ON\n");
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Auto Fix Failed Elements    :  OFF\n");
    ...
    if ( pzBatchRunParams->fCriteriaFile==(int)true )
    {
        ...
        if ( iRc==0) 
        {
            ...
            Session *theSession = Session::GetSession();
            CAE::CaePart* caeWorkPart(dynamic_cast<CAE::CaePart*>(theSession->Parts()->BaseWork()));
            CAE::ModelCheck::ElementQualitySetting *pEQValues(dynamic_cast<CAE::ModelCheck::ElementQualitySetting *>(caeWorkPart->ElementQualitySettings()->GetElementQualitySetting(scSolverName)));

            if( pEQValues )
            {
                double criteriaValue = 0.0;

                CAE::ModelCheck::ITestValue* pEQTestValue = NULL;

                // always check use non user specific
                pEQValues->SetUseElementSpecificValue(false);

                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeAspectRatio );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Aspect_Ratio (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                    else
                    {
                        pEQTestValue->SetDoTest(false);
                    }
                }


                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeJacobianZero );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Jacobian (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeTetCollapse );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Tet_Collapse (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                /* === 2D mesh related === */
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeWarpFactor );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Warp (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeSkewAngle );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Skew (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeTaper );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Taper (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeInteriorAngleMinimum );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_2D_MinAngle (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeInteriorAngleMaximum );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_2D_MaxAngle (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                ...
                pEQTestValue = pEQValues->GetTestValueByType( CAE::ModelCheck::TestValueTypes::TestTypeJacobianRatio );
                if( pEQTestValue )
                {
                    if (meshCriteriaReader.Get_Jacobian_Ratio (&criteriaValue))
                    {
                        pEQTestValue->SetDoTest(true);
                        pEQTestValue->SetCriteriaValue(CAE::ModelCheck::TestValueTypes::CriteriaTypeError, criteriaValue, NULL );
                    }
                                                 ...
                                                 UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams)
{
    ...
    /* FEM is already open and is work part */
    Session      *theSession = Session::GetSession();
    CAE::FemPart *workFemPart(dynamic_cast<CAE::FemPart *>(theSession->Parts()->BaseWork()));
    CAE::FemPart *displayFemPart(dynamic_cast<CAE::FemPart *>(theSession->Parts()->BaseDisplay()));
    CAE::FemPart *femPart1(dynamic_cast<CAE::FemPart *>(workFemPart));
    ...
    int iExpectedStrLen = (int)(strlen(JT_MESH_file_Name)+20);
    char* jt_file_path_name = (char*)UF_allocate_memory((iExpectedStrLen)*sizeof(char), &mem_error);
    if (mem_error != 0)
    {
        iRc = mem_error;
        return iRc;
    ...
    if (strlen(scPartDirectoryPath))
    {
        ...
        strcpy( jt_file_path_name, scJT_Directory);
        iRc = uc4560 (jt_file_path_name, 0 );
        if ( iRc ==0 )
        {
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "WARNING : Deleting pre-existing file found with the same name. \n" );
            /* File exists : So delete it from the system */
            iRc = uc4561 (jt_file_path_name, 0 );
            if ( iRc!=0 )
            {
                /* Could not delete the file : Error out */
                report_msg("Error : FEM file already exists\n");
    ...
    //        strcpy( jt_file_path_name, "./" );
    //}
    //strcat( jt_file_path_name, JT_MESH_file_Name );

    NXOpen::NXString nxStr( jt_file_path_name );
    if (femPart1!=NULL)
        femPart1->ExportJt(nxStr);
    else
    {
        /* Report error : we could not write JT.  should never come in here */
        write_info_fprintf ( true, UF_UG__BatchMesh_LOG,  "Error attempting to write JT from mesh: %s   \n\n",JT_MESH_file_Name);
    }
                                        ...
                                        int  *pfDisplayCommandHelp)
{
    ...
    *pfDisplayCommandHelp = false;
    ...
    /* Parameter file name */
    if (uc4620( UF_UG_BATCH_MESH__PARM_param_file , fspec_buffer)<=0)
    {
        pzBatchRunParams->fParamFile = false;
        strcpy(pzBatchRunParams->param_file_name, "");
    }
    else
    {
        pzBatchRunParams->fParamFile = true;
        strcpy(pzBatchRunParams->param_file_name, fspec_buffer);
    }

    /* Criteria file name */
    if (uc4620( UF_UG_BATCH_MESH__PARM_criteria_file, fspec_buffer)<=0)
    {
        pzBatchRunParams->fCriteriaFile = false;
        strcpy(pzBatchRunParams->criteria_file_name, "");
    }
    else
    {
        pzBatchRunParams->fCriteriaFile = true;
        strcpy(pzBatchRunParams->criteria_file_name, fspec_buffer);
    }
    ...
    /* Solver name */
    if (uc4620( UF_UG_BATCH_MESH__PARM_solver_name , fspec_buffer)<=0)
    {
        /* Default solver */
    ...
    {
        strcpy(pzBatchRunParams->Solver_Name, fspec_buffer);
    }
