...
static void do_ugopen_api(void)
{
  ...
  if ( strlen(message) < MAX_LINE_BUFSIZE )
  {
      strncat(message, title1, MAX_LINE_BUFSIZE-strlen(message));
  }
...
/* Display the attribute's value */
  switch ( value1.type ) {
    ...
    case UF_ATTR_string  :
      strncpy(sval, value1.value.string, UF_ATTR_MAX_STRING_BUFSIZE-1);
      sprintf(buffer, "String Attribute : %s\n", sval);
      UF_UI_write_listing_window(buffer);
      UF_free(value1.value.string);
      break;
    case UF_ATTR_reference :
      strncpy(sval,value1.value.reference,UF_ATTR_MAX_STRING_BUFSIZE-1);
