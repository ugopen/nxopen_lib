...
void ufusr(char *param, int *retcod, int param_len)
{
   int status = 0;
   char stat_msg[133];
   tag_t wcs, arc1_tag, arc2_tag, arc3_tag;
   ...
   if (!status)
   {
      ...
      /* create a full circular centerline: 3 point method */
      status = UF_DRF_create_3pt_cline_fcir(3, objects, &cl1_tag);
   }

   if (!status)
      /* create a partial circular centerline: 3 point method */
      status = UF_DRF_create_3pt_cline_pcir(3, objects, &cl2_tag);

   if (!status)
      /* create a full bolt circle centerline: 3 point method */
      status = UF_DRF_create_3pt_cline_fbolt(3, objects, &cl3_tag);

   if (!status)
      /* create a partial bolt circle centerline: 3 point method */
      status = UF_DRF_create_3pt_cline_pbolt(3, objects, &cl4_tag);

   printf("UF_DRF_create_3pt_cline_fcir,_pcir,_fbolt,_pbolt ");
   if (status)
   {
