#ifndef NXOpen_FIELDS_VECTORFIELDWRAPPER_HXX_INCLUDED
#define NXOpen_FIELDS_VECTORFIELDWRAPPER_HXX_INCLUDED
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Fields_VectorFieldWrapper.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/NXObject.hxx>
#include <NXOpen/libnxopencpp_exports.hxx>
namespace NXOpen
{
    namespace Fields
    {
        class VectorFieldWrapper;
    }
    class Expression;
    namespace Fields
    {
        class Field;
    }
    class NXObject;
    namespace Fields
    {
        class _VectorFieldWrapperBuilder;
        class VectorFieldWrapperImpl;
        /** This class defines a vector value that is internally 
                backed up by a (optionally scaled) field or three expressions. */
        class NXOPENCPPEXPORT  VectorFieldWrapper : public NXObject
        {
            private: VectorFieldWrapperImpl * m_vectorfieldwrapper_impl;
            private: friend class  _VectorFieldWrapperBuilder;
            protected: VectorFieldWrapper();
            public: ~VectorFieldWrapper();
            /** Sets the implementation of the wrapper to the specified expressions  <br> License requirements : None */
            public: void SetExpressions
            (
                const std::vector<NXOpen::Expression *> & expressions /** existing expressions that will be this wrapper's value */
            );
            /** Returns the indicated implementation if the wrapper is backed up by expressions; 
                        NULL otherwise  @return  existing expression or NULL  <br> License requirements : None */
            public: NXOpen::Expression * GetExpressionByIndex
            (
                int index /** 0, 1 or 2 */
            );
            /** Sets the implementation of the wrapper to the specified field  <br> License requirements : None */
            public: void SetField
            (
                NXOpen::Fields::Field * field /** an existing field that will be this wrapper's value */,
                const std::vector<double> & scaleFactors /** the field will be multiplied by this scale factor when being evaluated */
            );
            /** Returns the implementation if the wrapper is backed up by a field; 
                        NULL otherwise  @return  existing field or NULL  <br> License requirements : None */
            public: NXOpen::Fields::Field * GetField
            (
            );
        };
    }
}
#undef EXPORTLIBRARY
#endif
