...
void ufusr(char *param, int *retcod, int param_len)
{
   ...
   char  stat_msg[133];
   tag_t elems[NUM_OBJECTS], circles[NUM_MATERIAL];
   tag_t wcs, crosshatch_tag, view_tag = NULL_TAG;
   ...
   if (!status)
   {
      /* create some circles to crosshatch */
      UF_CSYS_ask_wcs(&wcs);
   ...
   if (!status)
   {
      ...
      mpi[68] = 3;
      /* set crosshatch angle */
      mpr[11] = 30.0;
      /* set crosshatch distance */
      mpr[12] = 0.6;
      /* set crosshatch tolerance */
      mpr[13] = 0.001;
      ...
      for (i = 0; i < NUM_MATERIAL; i++)
      {
         /* set the crosshatch material */
         mpi[31] = i + 1;
         /* set the module parameters arrays */
         uc5521(mpi, mpr, rad_symbol, dia_symbol);

         elems[0] = circles[i];
         status = UF_DRF_create_crosshatch(num_islands, num_elements,
                  elems, view_tag, &crosshatch_tag);
         if (status)
            break;
      }
   }

   printf("UF_DRF_create_crosshatch ");
   if (status)
   {
