...
/*   switch name                min   handler function          multi seen
    ...
    {"pim",                       3,  ignore_arg,               true, false,
     "=yes to initialise Teamcenter Integration for NX rather than native NX"}, /* multi is harmless, though not useful */
    ...
    {"validation_mode",          12,  validation_mode,         false, false,
     "=off|import_from_part|run_validation|run_validation_hybrid specifies how to import validation results\n\tthis switch must be positioned before the -assembly switch, -part switch and -dryrun switch.\n\t Default is off"
    },
    {"validation_rule",          12,  validation_rule,         false, false,
    "=<validation rule dataset name or validation rule file name> speicifies the dataset or native file name used to qualify the validation results to be imported.\n\t this switch must be specified when -validation_mode is not off. \n\t the name should be in CLI format. "
    },
    {"abort_import_on_fail",  12,  abort_import_on_fail,         false, false,
     "=no|yes speicifies whether to abort the part importing when validation results do not sastify the validation rule.\n\t Default is no"
    },
    {"treat_validation_warning_as_pass",  18,  treat_validation_warning,         false, false,
      "=yes|no speicifies how to treat the validation result status of pass with warning.\n\t Default is yes"
    },
    {"treat_validation_outdated_as_pass",  18,  treat_validation_outdated,         false, false,
    "=no|yes speicifies how to treat the validation result status of pass but outdated.\n\t Default is no"
    },
    ...
    {"associated_file_root_dir",  4,  associated_file_root_dir, false, false,
     "=<directory> specifies default Teamcenter Associated file directory"},
    {"copy_associated_files",     6,  copy_associated_files,    false, false,
     "=yes|no specifies whether to copy Teamcenter Associated files\n\tdefault is yes"},
    ...
    {"default_owner",             9,  default_owner,            false, false,
     "=<user>:<group> specifies default Teamcenter user and group to own parts"},
    ...
    {"import_dfa_kf",             13, import_dfa_kf,            false, false,
     " specify this argument in the beginning of the argument list to import dfa files.\n\tExample to use import_dfa_kf\n\t\tug_clone -pim=yes -o=import -import_dfa_kf=dfa_only -part=<dfa_filename>\n."},
    {"import_dfa_list",           15, import_dfa_list,           false, false,
     " specify this option to import the dfa filenames listed in an input file\n\tExample to use import_dfa_list\n\t ug_clone -pim=yes -o=import -import_dfa_kf=dfa_only -import_dfa_list -part=<input file list name>"},
    ...
    {"attach_log_file",          6,  attach_log_file,           false, false,
     " specifies that the clone log file will be attached as named reference to root nodes"}, /* NXDM MultiCad Clone (NXDM50101) project */
    ...
    else
    {
        ...
        if( !is_the_value_dfa( switch_value, UF_KF_UGMGR_EXPORT_DFA_IN_ASSEMBLY ) )
        {
            ...
            if(rc != 0)
            {
                ...
                if (display_rc != 0)
                    strcpy(display_name, switch_value);
...
static int ug_legacy_trans(const char input[MAX_FSPEC_SIZE+1], char output[MAX_FSPEC_SIZE])
{
    ...
    char *file_type = NULL;
    char *file_name = NULL;
    char internal_form[MAX_FSPEC_SIZE+1] = {(char) 0};
    int n_attributes;
...
extern void UFD_clone_main(int argc, char **argv)
{
    ...
    */
    for( i = 1; i < argc; i++ ) 
    {
        ...
        if( strcmp(help_descriptor->switch_name, "help") == 0 )
        {
        ...
        for (i = 1; i < argc; i++)
        {
            ...
            if ( descriptor != NULL ) 
            {
               if (strcmp( descriptor->switch_name, "password" ) == 0 ) 
               {
                    ...
                    strcpy(strVal, find_value(argv[i]));
