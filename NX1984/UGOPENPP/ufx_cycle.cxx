...
#ifdef _MSC_VER
#define IMPL_CYCLE // Used to properly define CLASS_DECL_CYCLE in ufx_cycle.hxx
#endif
#include <unidefs.h>
...
void Cycle::askHistoryReport ( const UgPart *part )
{
    ...
    for ( int current_history = 0; current_history < n_histories; current_history++ )
    {
        ...
        struct tm *time_ptr;
        char       t_buffer [  UF_UI_MAX_STRING_BUFSIZE ];
        char       time_buffer [ 21 ];
        ...
        STR_char_snprintf ( t_buffer, UF_UI_MAX_STRING_BUFSIZE, UF_UI_MAX_STRING_NCHARS, "%7d  %-15.15s  %-7.7s  %-20.20s  %s", version_number, time_buffer, machine, user, program );
