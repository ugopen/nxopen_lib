...
Input to:
  ...
  UF_ASSEM_add_part_to_assembly2,
  UF_ASSEM_reposition_instance,
  UF_ASSEM_reposition_instance2
Output from:
  UF_ASSEM_ask_component_data

The CSYS matrix consists of 6 or 9 reals, and the origin consists of 3 reals.
The CSYS matrix containing 6 reals specifies the x and y orientation of the 
component in assembly space when adding a component (UF_ASSEM_add_part_to_assembly)
or repositioning a component (UF_ASSEM_reposition_instance).
The CSYS matrix containing 9 reals specifies the x, y and z orientation of the 
component in assembly space when adding a component (UF_ASSEM_add_part_to_assembly2)
or repositioning a component (UF_ASSEM_reposition_instance2). This 9 real CSYS 
matrix may contain a reflection (i.e. the determinant of the matrix can be -1).

For a 6 real CSYS matrix, the z orientation is generated from the cross-product of 
the x and y vectors to give a full orientation matrix.  This matrix is the one 
returned from UF_ASSEM_ask_component_data.

When adding a component with a reference set, the reference set itself may have 
a coordinate system attached to it. This is subtracted from the matrix and origin
passed in with the effect that the component is positioned by its reference set
origin and orientation, instead of its absolute origin and orientation.
...
scale which could be either 1 or 25.4 or 0.0393
...
This function differs from UF_ASSEM_add_part_to_assembly only in that it allows
the orientation matrix of the new instance to contain a reflection.

Environment: Internal  and  External

See Also: 

History: Originally released in 1847
***************************************************************************/
extern UFUNEXPORT int UF_ASSEM_add_part_to_assembly2(
    tag_t                   parent_part,        /* <I>
                                                   Tag of part to add instance to.
                                                */
    const char              *part,              /* <I>
    ...
    double                  origin[3],          /* <I>
                                                   ...
                                                   is to be created
                                                */
    double                  csys_matrix[9],     /* <I>
                                                   Orientation of the instance.  This is allowed to be a
                                                   reflection matrix.
                                                */
    int                     layer,              /* <I>
                                                   -1 Means use the original layers.
    ...
    tag_t                   *instance,          /* <O>
                                                ...
                                                */
    UF_PART_load_status_t   *error_status       /* <OF>
                                                User allocated structure consisting of
                                                names and associated error codes. The
                                                allocated arrays must be freed with
                                                UF_free_string_array and UF_free.
                                                See the description of the
...
See Also:  UF_ASSEM_reposition_instance2
...
/*****************************************************************************
Reposition an existing instance to a new position/orientation where the orientation
may include a reflection.

Environment: Internal  and  External

See Also:  UF_ASSEM_reposition_instance

History: Originally released in 1847
***************************************************************************/
extern UFUNEXPORT int UF_ASSEM_reposition_instance2(
    tag_t instance,                 /* <I>
                                       tag of instance to reposition
                                    */
    double new_origin[3],           /* <I>
                                       New orientation of the instance
                                    */
    double new_csys_matrix[9]       /* <I>
                                       New coordinate system matrix which may include a reflection
                                    */
);
...
This routine together with  UF_ASSEM_restore_work_part_context_quietly 
replaces UF_ASSEM_set_work_part_quietly.
...
This routine together with  UF_ASSEM_set_work_part_context_quietly
replaces UF_ASSEM_set_work_part_quietly.
...
/*****************************************************************************
Returns the current settings of the assembly options into the options
structure supplied by the user.  For NX12 and later.

Environment: Internal  and  External

See Also: Refer to  UF_ASSEM_load_options_t

History:
***************************************************************************/
extern UFUNEXPORT int UF_ASSEM_ask_assem_load_options(
UF_ASSEM_load_options_t *options  /* <O>
                                Pointer to an assembly options structure
                                */
);
...
Set the current settings of the assembly options. To assure that your
program continues to work correctly with future versions of Open
API, we recommend that you call UF_ASSEM_ask_assem_load_options to
initialize your options structure before setting the values you desire to
set/modify. New options may be added to the UF_ASSEM_load_options_s
structure in the future.   For NX12 and later.
...
History:
***************************************************************************/
extern UFUNEXPORT int UF_ASSEM_set_assem_load_options(
UF_ASSEM_load_options_t * options  /* <I>
                                Pointer to an assembly options structure
                                */
);
...
Unlike UF_ASSEM_ask_component_data, this method will not always return unit scale transformation. 

The element,
    transform[3][3]

may have either 1 or 25.4 or 0.0393 as below
    1     :  Component and its parent are in same units
    25.4  :  Component is in "mm" but parent is in "in"
    0.0393:  Component is in "in" but parent is in "mm"

when the scaling component is not 1, to convert the transform to unit scale transform, the origin of the component needs to be divided
by the scaling factor as follows:
    transform[0][3] = transform[0][3] / transform[3][3]
    transform[1][3] = transform[1][3] / transform[3][3]
    transform[2][3] = transform[1][3] / transform[3][3]

and set the value of element transform[3][3] to 1.
...
tag_t component ,/* <I>
                 ...
                 */
double transform[4][4]  /* <O>
                        a 4x4 non-distorting transform relative to root part occurrence
                        */
);
...
Return:
    Return code:
        0 = No error            non-0 = Error code

    Error code - use UF_get_fail_message to obtain the corresponding
    error message. Some possible error codes are:
    "The first parameter passed in was invalid." - The given component is NULL.
    "The second parameter passed in was invalid." - The given view is NULL.
...
Return:
    ...
    Error code - use UF_get_fail_message to obtain the corresponding
    error message. Some possible error codes are:
    "The first parameter passed in was invalid." - The given view is NULL.
...
/*******************************************************************************
    UF_ASSEM_add_snapshot_motion_to_sequence

    Creates a snapshot series motion given a matrix of transforms 
    (represented by num_snapshots x num_occs x 16). The newly created snapshot series 
    motion is added to the sequence after the specified step_to_insert_after. 
    However, if step_to_insert_after is NULL_TAG, the snapshot series will be added to 
    the beginning of the sequence. The tag of this new snapshot series is returned.
    ...
    History:
        Initially released in NX1872.
*******************************************************************************/
extern UFUNEXPORT int UF_ASSEM_add_snapshot_motion_to_sequence
(
    const char* snapshot_series_name,       /* <I> Name of the snapshot series. If NULL, the snapshot series will be given the default name "Snapshot Series" */
    int num_snapshots,                      /* <I> The number of snapshots (frames) to create for each part occ in the new snapshot series */
    int num_occs,                           /* <I> The number of part occs participating in the snapshot series motion */
    tag_t *occs,                            /* <I> Array of part occs participating in the snapshot series motion */
    double snapshot_series_xforms[],        /* <I> A flattened array of 3D matrix size i x j x 16 containing transforms for each part occurrence in each frame of 
                                                   motion, where i = number of snapshots (frames) and j = number of part occurences (components) and the 16 elements (k) 
                                                   are the flattened 4x4 transforms of the component. For example, [i0j0k0, ..., i0j0k15, i0j1k0, ..., i0j1k15, ...] */
    tag_t sequence,                         /* <I> Tag of the sequence in which to create the snapshot series motion */
    tag_t step_to_insert_after,             /* <I> Tag of the step after which to insert the snapshot series
                                               If NULL_TAG, the snapshot series step will be inserted at the beginning of the sequence */
    tag_t *snapshot_series                  /* <O> Returns the tag of the snapshot series created in the sequence */
);
...
extern UFUNEXPORT int UF_ASSEM_create_deformable_part( UF_ASSEM_deform_data_p_t data,  /* <I> */
                                            tag_t *deformable_feature                 /* <O> */   );/* <RETIRE_DOTNET> */
