...
Note that Step length (stol) is the maximum allowable length between two
points and it should be set depending on the dimension of the curve. If the 
curve is of a larger dimension and if we set a very tight stol, it might 
result in a large number of points, which might not be desired.

In order it to return the expected coordinates the displayed part must be 
...
Note: The data of the function output spline may be different from the 
input data when the state of knot_fixup or pole_fixup is 1. In these cases,
you can use UF_CURVE_create_spline to keep the same data.
...
/*==========================================================================
  ...
  OUTPUT-
  -------
  *uf_law_parms     = pointer for law_param data, To be freed from caller UF_MODL_free_law ( uf_law_parms );
...
/* ***************************************************************************
  ...
  OUTPUT-
  -------
       void **         uf_law_parms     = pointer for law_param data, To be freed from caller
                                          UF_MODL_free_law ( uf_law_parms );
...
extern UFUNEXPORT int UF_MODL_create_multi_transition_law (
    UF_STRING_t     spine,                  /* <I> */
    int             num_pts,                /* <I> */
    double          pt_xyzs[],              /* <I,len:3*num_pts> */
