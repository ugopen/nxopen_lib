...
-------------------------------------------------------------------------------
        Demonstration example for custom feature B-Surface UI code.
-------------------------------------------------------------------------------
...
This example demonstrates how a Custom feature "B-Surface" can be implemented.
...
--------------
	...
	2. BSurfaceUI.cpp
	3. BSurfaceUI.dlx	
    4. BSurfaceUI_CPP_ReadMe.txt
	
 The above files are located at $UGII_BASE_DIR/ugopen/SampleNXOpenApplications/C++/CustomFeatures/BSurface/BSurfaceUI.
...
------------
  ...
  Windows
  -------
   1. Using Visual Studio - create a new project with name "BSurfaceUI" using the NX Open C++ Wizard.
   ...
   3. Add the BSurfaceUI.cpp and BSurfaceUI.hpp to Source Files by selecting the Project pull down and select "Add Existing Item...". Select the file 
      using the popup window.

   4. Build the internal image by going to the Build menu and selecting either 
      "Build Solution" or "Rebuild Solution".
 
   This will create an executable called BSurfaceUI.dll in the Debug directory.
   ...
   The uflink tool can also be used to create the executable by following these steps:
    ...
    ufcomp BSurfaceUI.cpp
      uflink UFDLL=BSurfaceUI.dll BSurfaceUI.obj

   This will create BSurfaceUI.dll in the current directory.
  ...
  -----
   ...
   This will create BSurfaceUI.so in the current directory.
   
----------------------------
Settings before Launching NX
---------------------------- 
The executable (BSurfaceUI.dll for Windows or BSurfaceUI.so for Linux) and BSurfaceUI.dlx must be placed in any one of the following locations mentioned below:
...
-----------------------
    ...
    4. This will launch the "B-Surface" dialog.

    5. Provide inputs as indicated in dialog.

    6. Dialog OK/Apply creates the B-Surface feature.
	
	7. Newly created B-Surface feature can be edited by RMB on Part Navigator feature node and choosing Edit with Rollback
