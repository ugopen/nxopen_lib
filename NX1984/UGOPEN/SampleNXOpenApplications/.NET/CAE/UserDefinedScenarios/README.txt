...
1. Averaging Scenario
    Scenario:  AveragingScenario.cs
    Data file: FunctionDataSample.unv

    This scenario is producing the arithmetic and geometric average for the values of the intersected independents data and it 
    displays them separately in two XY plots.

2. Scaling and Summation Scenario
Files required:
    Scenario:  ScalingAndSummationScenario.cs
    Data file: FunctionDataSample.unv
...
-----------
    ...
    2. Create new project as:
		...
		2.2. Select C# Projects using the NX Open AppWizard, Class Library Application, Enter
			 name as per the example scenario above. Click Next
		...
		2.4. Select to activate the application automatically(Startup) and to unload the application when NX session terminates. 
		     Click Finish.

    3. Remove the initial C# file from the project. Highlight it in the Solution Explorer and use the 3rd mouse button
    ...
    4. In the Solution Explorer highlight the project name and use the 3rd mouse 
       button and drag to Add.  Choose 'Add Existing Item'. Select the file .cs example files mentioned above and add it. 
	  
    5. Add the following reference,
   		5.1. NX Open References
        If needed add references to NXOpen.dll, NXOpen.UF.dll,NXOpen.UI.dll, and NXOpen.Utilities.dll.
        To do this, right click on "References" under the project in Solution Explorer. 
        Pick "Add Reference..." and then navigate to the location of the dlls. 
        These dlls can be found in %UGII_BASE_DIR%\nxbin\managed.

    6. Go to, Build Menu -> Build Solution or Ctrl + Shift + B or go to Solution Explorer, right click on the project 
       name and select Build.

    7. This will create dynamic library called as named initially in the Release or Debug directory as per selected configuration.

    At this point the resulting .dll can be added in the startup folder indicated by the UGII_USER_DIR or a startup folder as pointed
