...

{
    ...
    if (OldChildTopology->u.user_ptr != NULL && NewParentTopology != NULL)
        {
    ...
    *NewChildTopology = (UF_BREP_topology_t *) malloc (sizeof (UF_BREP_topology_t));
    ...
    if (OldParentTopology != NULL && NewParentTopology != NULL)
        {
...

{
    ...
    strncpy (NewPartName, ArgV[1], MAX_FSPEC_BUFSIZE - 1);
    if ( strlen(NewPartName) < MAX_FSPEC_BUFSIZE)
    {
       strncat (NewPartName, "_new", MAX_FSPEC_BUFSIZE-strlen(NewPartName));
    }
    ...
    while (1)
        ...
        UF_BREP_topology_t  *MakeBodyRoot = NULL;
        ...
        if (Type == UF_MODL_SHEET_BODY && MakeBodyRoot != NULL)
            while (MakeBodyRoot->type != UF_BREP_FACE_TOPO && MakeBodyRoot->children != NULL)
