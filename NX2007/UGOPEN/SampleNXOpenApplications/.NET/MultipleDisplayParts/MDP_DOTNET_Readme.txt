...
The VB.Net file named MDP_On.vb will enable MDP, and then display an NXMessageBox with the MDP status 
showing that MDP is enabled. The MDP_Off.vb file will disable MDP, and then display an NXMessageBox with 
...
--------------
 ...
 All the above files are located at $UGII_BASE_DIR/ugopen/SampleNXOpenApplications/.NET/MDP
...
----------- 
 ...
 Windows
 ---------
   This example can be run using the Journal Manager or setting up the VB files to be 
   run off a button.
...
--------------------------
  ...
  4. Run the MDP_Off.vb journal and observe that MDP will be disabled with only one DisplayedPart.
  
  5. Attempt to Display the other part (ex. Using Window pull-down) and see that NX shows only one DisplayedPart.
  
  5. Run the MDP_On.vb journal and observe that MDP will be enabled with NX showing only one DisplayedPart.
...
Note:
-----
 Please note if this is built into DLL, NXOpen application must be signed before its release. If not signed the application may not get executed.
