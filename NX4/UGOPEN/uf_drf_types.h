...
UG/Open typedefs and macros that are specific to the DRF module. 
...
#define SYMBOL_NAME_SIZE          8  /* maximum symbol name size, not 
                                        including "\0"      */
#define SYMBOL_FILE_NAME_SIZE    UF_CFI_MAX_FILE_NAME_LEN  /* maximum symbol file name size, not 
                                        including "\0" */
...
#define MAX_LINE_LENGTH         132  /* maximum line length, not including "\0"
                                     */
...
#define UF_DRF_MAX_HATCH_FILE_NAME_LEN UF_CFI_MAX_FILE_NAME_LEN
#define UF_DRF_MAX_HATCH_MATERIAL_NAME 30
...
struct UF_DRF_text_s {
        char         *user_dim_text;      /* user supplied dimension text 
                                          string 
                                          */
        int           lines_app_text;     /* number of lines of appended text
                                          */
        char        (*appended_text)[133];/* <len:lines_app_text> appended text array 
                                          */
};
...
struct UF_DRF_draft_aid_text_s
{
        int     num_chars;              /* number of characters */
            /* The 132 byte maximum line restriction has been removed.
               ...
               since string will be removed in a future Unigraphics release.
        ...
        char    string[MAX_LINE_LENGTH+1];      /* 132 max, plus one end of 
                                           line char */
...
{ 
        ...
        int                          num_leaders; /* number of leaders     */
        UF_DRF_leader_info_t         *leader_info;/* <len:num_leaders> leader information    */
        int                          num_arrows;  /* number of arrowheads */
...
/****************************************************************
 Ordinate dimension positive quadrrnt                          
*****************************************************************/
        ...
        UF_DRF_ordorigin_display_type_t name_display; /* Ordinate dimension
                                        origin name display type. See
                                         UF_DRF_ordorigin_display_type_t
                                                      */ 
        char                            origin_name[MAX_LINE_LENGTH+1]; /* The name of origin */
...
#define UF_DRF_DIAMETER_RADIUS_SYMBOL_MAX_LENGTH 27 
...
struct UF_DRF_diameter_radius_preferences_s
{
     UF_DRF_diameter_symbol_t         
           diameter_symbol_type;      /* standard diameter symbol or
                                         user-defined */
     char  
	   diameter_symbol[UF_DRF_DIAMETER_RADIUS_SYMBOL_MAX_LENGTH + 1]; 
                                      /* text for user-defined diameter symbol*/
     UF_DRF_radius_symbol_t           
           radius_symbol_type;        /* standard radius symbol or
                                         user-defined */
     char  
	   radius_symbol[UF_DRF_DIAMETER_RADIUS_SYMBOL_MAX_LENGTH + 1];
...
struct UF_DRF_hatch_fill_preferences_s
{
     ...
     char    hatch_file[UF_DRF_MAX_HATCH_FILE_NAME_LEN+1]; /* name of file containing crosshatch 
                                     patterns */
     char    hatch_material[UF_DRF_MAX_HATCH_MATERIAL_NAME+1]; /* name of crosshatch pattern */
...
struct UF_DRF_weld_sym_info_s
{
    ...
    UF_DRF_weld_size_code_t     size_code;    /* size code - applicable
                                                 only for ISO and DIN */
    char                        angle[MAX_LINE_SIZE+1]; /* Groove Angle or Countersink 
                                                 Angle */
    char                        num_root_depth[MAX_LINE_SIZE+1]; /* Number of Welds or Root 
                                                 opening or Depth of Weld as
                                                 the case may be */
    char                        size[MAX_LINE_SIZE+1]; /* size of the weld */ 
    char                        pitch[MAX_LINE_SIZE+1]; /* pitch of the weld */
};
...
struct UF_DRF_weld_symbols_s
{
    ...
    int                           num_lines;       /* number of lines of
                                                      reference text */
    char                          symbol_standard[MAX_ENTITY_NAME_SIZE+1]; /* standard for the weld 
...
typedef struct UF_DRF_custom_symbol_text_s { 
    ...
    char *title;                                /*The title of this text field.  
                                                  This title display in the dialog 
                                                  when the symbol is picked interactively
                                                  for editing.*/
...
typedef struct UF_DRF_object_assoc_data_s
{
    ...
    UF_DRF_object_assoc_point_type_e point_type;
    UF_DRF_object_assoc_line_type_e line_type;
