...
can perform within Unigraphics plus query routines.
...
The information maintained within the Unigraphics part file for each
...
normal Unigraphics object and can not be used by other Open C
...
const char * part_name ,/* <I>
                        Name of the new part.  If there is not a directory path
                        specified, the name of the new part must not already
                        exist in the directory where the the NX session or
                        external program was started.
                        */
int   units ,/* <I>
...
Unigraphics part with a .prt extension. The file name of the new NX
...
part name to the part name it has been stored with. The
component being opened-as must be unloaded.  The new part being
substituted for it can either be loaded or not.  The user
allocated structure <error_status> will be filled with the
name and associated error code of the loaded part. The
allocated arrays must be freed.
...
in Unigraphics. Therefore, it is possible to consume large amounts
...
UF_PART_cleanup_highlight - In internal Open API, turns off
                            ...
                            objects in the root part.  In external
                            Open API, this option is a no-op.
...
should not be more than 132 characters in length.
...
extern UFUNEXPORT int UF_PART_set_customer_area(
    ...
    const char *customer_area  /* <I>
                               ...
                               string is currently limited to 132 characters.
                               */
);
/*****************************************************************************
...
string should not be more than 132 characters in length.
...
const char * description  /* <I>
                          ...
                          currently limited to 132 characters.
                          */
);
...
/*****************************************************************************
Returns the version number of last significant change for the given part.
...
done separately on the new part using UF_ASSEM_set_work_part_quietly,
UF_PART_save and UF_PART_close.
...
leaf name.  In NX3 and beyond, use UF_PART_name_for_display() on the leaf
...
See Also: UF_PART_name_for_display
...
const tag_t part_tag,         /* <I> The tag of the part you want to update the facets of */
...
extern UFUNEXPORT int uc5001(char *cp1);

/* uc5003 file current part - OLD NAMING CONVENTION */
extern UFUNEXPORT int uc5003(char *cp1, int ip2);

/* uc5000 check for active part  - OLD NAMING CONVENTION */
extern UFUNEXPORT int uc5000(char *cr2);
