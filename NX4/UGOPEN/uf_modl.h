...
tag_t feature_obj_id ,/* <I>
                      ...
                      */
double  dir_x[] ,/* <O,len:3>
                  X direction
                  */
double dir_y[]  /* <O,len:3>
                 Y direction
                 */
);
/********************************************************************
...
/******************************************************************************
Inquires the periodicity of the underlying surface a face.
Open face may be periodicity.
...
/******************************************************************************
Calculates maximum curvature (where this maximum curvature occurs) of a
curve or a face by inputting the object's (curve or face) identifier, the
range where the maximum curvature is to be found, and the curvature type if
the input object is a face.
...
tag_t eid ,/* <I>
           Object identifier of the entity (curve or face)
           */
double range[4] ,/* <I>
              Object's relative parameter range (in percentage) to be searched.
                   i.e., 0.0 <= umin <= umax <= 100.0
                For curves: range[0]=umin, range[1]=umax.
                For surfaces: range[0-1]= umin & umax
                range[2-3]=vmin & vmax
                */
int curva_type ,/* <I>
                ...
                objects.
                */
double max_curva[4] ,/* <O>
                   Data about where the maximum curvature occurred
                   For curve objects
                   max_curva[0] = u
                   [1-3] = position
                   [4] = curvature value
...
int * status  /* <O>
              For curve objects
              0= max_curva[] is returned.
              1=constant curvature object encountered. only
              return the constant value in max_curva[4]
              */
);
...
int edit ,/* <I>
          ...
          are returned.
          */
double fixed[] ,/* <O,len:2>
                 Dimension attachment point on the part
                 */
double feature[] ,/* <O,len:2>
                   Dimension attachment point on the feature
                   */
double fixc[] ,/* <O,len:2>
...
int edit ,/* <I>
          ...
          are returned.
          */
double fixed1[] ,/* <O,len:2>
                  Dimension attachment point on the part.
                  */
double fixed2[] ,/* <O,len:2>
                  Point on the part that along with the fixed1 point,
                  defines the line that the dimension is perpendicular
                  to.
                  */
double feature1[] ,/* <O,len:2>
                    Dimension attachment point on the part.
                    */
double feac[] ,/* <O,len:2>
...
tag_t obj_tag ,/* <I>
               Tag of solid body, face or edge, or ug curve
               */
double tolerance ,/* <I>
...
the offset feature. This routine only works on planar faces.
...
extern UFUNEXPORT int UF_MODL_edit_points_parms(
    tag_t  feature_tag,         /* <I> Feature tag */
    int    num_points,          /* <I> Number of points */
    tag_t *points               /* <O,len:num_points> Array of point tags */
);
...
/******************************************************************************
Sews sheet bodies or faces of solid bodies together.
...
tag_t targets[] ,/* <I,len:num_targets>
                 Array of target tags ( sheet body when type_body = 0 or face(s) from a solid body when type_body = 1 )
                 */
int num_tools ,/* <I>
               Size of the tool array
               */
tag_t tools[] ,/* <I,len:num_tools>
               Array of tool tags ( sheet body when type_body = 0 or face(s) from a solid body when type_body = 1 )
               */
double tolerance ,/* <I>
                  Tolerance value
                  */
...
tag_t so_csys, /* <I> This is the tag of a smart CSYS to be used for a
                      ...
                      be the WCS.
                */
char  factors[3][UF_MAX_EXP_LENGTH], /* <I,len:UF_MAX_EXP_LENGTH> An array of strings that define the
...
(
    tag_t set,      /* <I>  The tag of the feature set */
    int *hide_state /* <I/O> Logical value for hide state TRUE/FALSE
...
(
    ...
    int *flag      /* <O> Logical TRUE/FALSE if tag is a member of a set */
);
...
/************************************************************************
This function replaces the features in original_features[] by copies of 
the features in replacement_features[] if original_features[] are 
features on bodies, curve features and datum features. The children of 
the original_features[] will be reparented to the copies of the 
replacement_features[].

This function also replaces the features in original_features[] by the 
features in replacement_features[] if the feature in original_features[] 
is a datum plane or datum axis feature and the replacement_features[] is 
also a datum plane or datum axis feature.  The children of the 
original_features[] will be reparented to features in 
replacement_features[].

The original features will be deleted in both the above cases.
...
is before the earliest feature in the original_features[].  For datum 
features, the replacement_features[] must have a smaller timestamp than 
the earliest child feature of the feature in original_features[].
...
extern UFUNEXPORT int UF_MODL_replace_features  (
    ...
    logical return_map,             /* <I> return_map = true - return in
                                    ...
                                    */
    logical retain_replacement,     /* <I> retain_replacement = false - delete
                                       replacement features after replacing
                                       original features with copies of
                                       replacement features.  Does not apply if
                                       the original features are datums.
                                    */
