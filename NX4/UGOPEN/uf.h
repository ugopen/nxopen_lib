...
File Description :
   UG/Open general interface.  These are general purpose functions that
   will be common to most UG/Open programs, as well as specialized functions
...
/*****************************************************************************
    Unique identifiers for standard UG applications
*****************************************************************************/
...
#define UF_APP_SFEM          (68)     /* CAE - SFEM (Basic FEM) */
...
#define UF_APP_ADVFEM        (81)     /* CAE - Advanced FEM */
...
#define UF_APP_DESFEM        (89)     /* CAE - Designer FEM */
...
/****************************************************************************
*   Get the information about a UG/Open Program
*
*   This routine can be used to gather the system information of a UG/Open
...
*   Start a timer.
*
*   This routine can be used to gather timing information about a UG/Open
...
*
*   This routine is used to gather timing information about a UG/Open program,
...
/*****************************************************************************
 * Frees the memory allocated and returned by a UG/Open API routine.
 *
 * Return : void
 *
 * Environment: Internal  and  External
 ...
 * the array.  Various UG/Open API routines malloc space for an array
 ...
 * Environment: Internal  and  External
 *
 * See Also:  UF_TAG_ask_handle_of_tag
 *
 * History:
 ****************************************************************************/
 ...
 * history area.
 *
 * Interactively (Internal mode), when you are in UGII, the program
...
/****************************************************************************
 * Initializes the UG/Open API environment and allocates a UG/Open
 * API execute license. If you do not call UF_initialize, then subsequent
 * calls to UG/Open API fail with an error code of
...
/****************************************************************************
 * Checks if the UG/Open API environment has been successfully
 * initialized.
 *
 * Return:
 *        return code:
 *          0 = UG/Open API is not initialized.
 *          1 = UG/Open API has been properly
 *              initialized.
 *
 * Environment: Internal  and  External
 *
...
/****************************************************************************
 * Terminates the UG/Open API environment and deallocates a
 * UG/Open API execute license. If UF_terminate is not called, the
 * UG/Open API execute license may not be freed. Once UF_terminate
 * is called in an external UG/Open API program, the program should
 ...
 *
 * While a GRIP program executes, it can have access to a UG/Open API function
 * arguments and their contents. Argument passing to the GRIP program occurs
 * through UFARGS. After GRIP program termination, the UG/Open API program has
 ...
 * UF_call_grip, UG will look in the current directory for the grip program.
...
/*****************************************************************************
 * Checks to see if a UG/Manager process is
 * active. It returns true if UG/Manager is active.
 *
 * Environment: Internal  and  External
 *
 * See Also:
...
extern UFUNEXPORT int UF_is_ugmanager_active(
    logical    *is_active    /* <O>
                     Returns true if UG/Manager is active
                         */
);
/****************************************************************************
 ...
 * addition to the processing that UG does, during that particular operation.
 */
 ...
 *       it makes any UG/Open function calls
 *
...
extern UFUNEXPORT int UF_add_callback_function (
    ...
    UF_callback_fn_t fn,              /* <I>
                                      ...
                                      makes any UG/Open function calls.  It
 ...
 * data structures outside of UG with UG. Note that the routines registered
 ...
 *   . The routines registered can not call any other UG/Open routines
...
/****************************************************************************
*   Set a flag that determines if a UG/Open executable will issue
*   warnings when a function that is scheduled to be retired is called.
*   This can be used to alert the UG/Open programmer that modification to
*   their program is required.
*
*   When routines are planned to be removed from the UG/Open interface, the
*   prototype will be moved to uf_retiring.h, along with a description of
*   what the UG/Open programmer should do.  A warning can be issued when
*   any of these routines are called by a UG/Open application.  This routine
*   allows the UG/Open programmer to set the value of this flag.
*
...
*
*   This message can be used to print run time information about the UG/Open
...
char *file_spec,       /*<I>
                       ...
                       must be less than UF_CFI_MAX_PATH_NAME_LEN characters.
                       */
int ip3                /*<I>
...
/****************************************************************************
*   Query the current UG/Open program environment
*
*   Environment: Internal and External
*
*   Returns:
...
int *ir1 /* <O>
            The environment that the UG/Open program is running in.
            0 is External UG/Open, 1 is Internal UG/Open.
         */
);
...
extern UFUNEXPORT int uc4403(char *cp1);
...
extern UFUNEXPORT int uc4404(char *cp1, int ip2);

/* uc4406 write character string and real to listing device set by uc4400
   if you only need to write the data to the listing window
   (and not a particular listing device set by uc4400)
   this function may be replaced by UF_UI_write_listing_window */
extern UFUNEXPORT int uc4406(char *cp1, double rp2);
