...
struct UF_MODL_features_s
{
    ...
    char        feat_type[MAX_LINE_SIZE+1];
} ;
...
{
    double      hit_point[3];  /* pt coord where ray hit the target */
    double      hit_normal[3]; /* normal of the target at the hit point */
    tag_t       hit_face;      /* face tag at the hit point */
    tag_t       hit_body;      /* body tag at the hit point */
} ;
...
struct UF_MODL_law_spine_pt_val_s
{
    ...
    char   value[MAX_LINE_SIZE+1]; /* The value assigned to the law at the
                                      given point. */
};
...
{
    char        value[MAX_LINE_SIZE+1]; /* The law evaluates to this constant
...
{
    char        start_value[MAX_LINE_SIZE+1]; /* The starting value. */
    char        end_value[MAX_LINE_SIZE+1];   /* The ending value. */
} ;
...
{
    char        parm[MAX_LINE_SIZE+1];    /* A character string containing the
                                             dependent variable of the equation
                                             which varies from 0 to 1 during
                                             evaluation of the law. */
    char        function[MAX_LINE_SIZE+1]; /* A character string containing
...
{
    char        value[MAX_LINE_SIZE+1]; /* A character string containing the
...
{
    char        start_value[MAX_LINE_SIZE+1]; /* A character string containing
                                                 the starting value desired. */
    char        end_value[MAX_LINE_SIZE+1]; /* A character string containing
...
{
  char    value[MAX_LINE_SIZE+1]; /* A character string containing the constant
...
} UF_MODL_face_blend_create_data_t, *UF_MODL_face_blend_create_data_p_t;
...
section_surface.polynomial_status = 0; make surface rational (exact)
                                    1; make surface polynomial (approximate)
...
section_surface.direction_indicator = 1; create surface in the direction of the
                                         face normal
                                    1-; create surface opposite the face normal
...
struct UF_MODL_offset_trans_faces_s
{
  ...
  char          distance[MAX_LINE_SIZE+1]; /* <len:MAX_LINE_SIZE+1>
...
{
    tag_t            cyl_face;          /* Cylindricla face to be threaded. */
...
{
    tag_t *frecs;                   /* <len:num_frecs> Array of UDF member frecs */
    int num_frecs;                  /* Number of UDF member frecs */
...
typedef struct UF_MODL_udf_mapping_data_s
{
    ...
    */  
    ...
    UF_MODL_udf_reverse_dir_t *reverse_objs_dir; /*<len:num_mapping_objs><len:num_refs> Rev. mapping obj. dir. flags */
...
{ 
    ...
    tag_t                          smart_csys;  
                                   /* The tag for a coordinate system, of which
                                      ...
                                      used if NULL_TAG */
    char                           offset_distance[UF_MAX_EXP_LENGTH];
                                   /* Offset distance */ 
    char                           offset_deviation[UF_MAX_EXP_LENGTH]; 
                                   /* Offset distance deviation */ 
    char                           stepover_distance[UF_MAX_EXP_LENGTH]; 
                                   /* Stepover distance */
...
{ 
	...
	char radius[UF_MAX_EXP_LENGTH];   /* The radius of the spherical corner */
...
typedef enum UF_MODL_sflange_dir_e 
{ 
...
typedef enum UF_MODL_sflange_trim_e
{
...
typedef enum UF_MODL_sflange_continuity_e
{
...
{ 
    int         shape_control;  /********************************** 
                                ...
                                teh rest of the data structure 
                                *******************************/ 
