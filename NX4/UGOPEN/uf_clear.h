...
These subroutines provide access to Clearance Analysis data within Unigraphics.
...
UF_CLEAR routines.
    ...
    . UF_CLEAR_err_name_exists: An attempt has been made to create a data set 
      with the name of an existing data set.

    . UF_CLEAR_err_cant_find_dataset: The named data set cannot be found.

    . UF_CLEAR_err_bad_list_num: An invalid object list number has been 
      supplied. The list number must be 1 or 2.

    . UF_CLEAR_err_bad_interf: The tag (or tags) specified does not define a 
      valid interference.

    . UF_CLEAR_err_bad_pair: The two object tags do not define a valid pair.

    . UF_CLEAR_err_bad_part: The tag specified was not a valid part.

    . UF_CLEAR_err_analysis_error: An error occurred during analysis.

    . UF_CLEAR_err_facet_error: A faceting error occurred during analysis.

    . UF_CLEAR_err_bad_dataset: The tag specified was not a valid clearance 
      analysis data set.

    . UF_CLEAR_err_bad_copy_mode: An invalid copy mode parameter was supplied 
      to a routine.

    . UF_CLEAR_err_bad_string: The text supplied is invalid (either too long or
      zero length).

    . UF_CLEAR_err_null_pointer: A null pointer has been passed to a routine.

    . UF_CLEAR_err_bad_list_type: The object list type parameter is nvalid.

    . UF_CLEAR_err_bad_analysis_mode: The analysis mode supplied to a routine 
      is invalid.

    . UF_CLEAR_err_bad_expression: The tag specified does not refer to a valid 
      expression.

    . UF_CLEAR_err_bad_list_data: The data in an object list is invalid.

    . UF_CLEAR_err_bad_exclude_rule: The exclusion rule supplied to a routine 
      is invalid.

    . UF_CLEAR_err_bad_msg_filter: The message filter supplied to a routine is 
      invalid.

    . UF_CLEAR_err_bad_layer: The layer number supplied to a routine is invalid.

    . UF_CLEAR_err_bad_color: The color index supplied to a routine is invalid.

    . UF_CLEAR_err_no_results: The data set in question does not contain any 
      analysis results.

    . UF_CLEAR_obj_not_avail: One or more objects were not available during 
      analysis because they were not loaded.
