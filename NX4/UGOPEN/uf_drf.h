/*******************************************************************************
             Copyright (c) 1998-2003,2004,2005 Unigraphics Solutions, Inc.
                       Unpublished - All Rights Reserved



File description:
    UG/Open interface to the drafting module. This file contains routines
    dealing with:
...
Return:
    ...
    UF_err_program_not_initialized - UG/Open API has not been initialized.
...
int num_lines_text ,/* <I>
                    ...
                    */
char text_string[][MAX_LINE_LENGTH + 1] ,/* <I,len:num_lines_text>
...
int *number_lines, /* <I>
                   ...
                   */
char text_array[][133] /* <I,len:number_lines> number_lines
...
if applicable, updating hidden line display.  This routine will update one view
(method>0 & view_name), all automatic views (method < 0), or all views
(method = 0) on the specified drawing.
...
If one view is specified (method > 0), then it will be updated, even if
it is up-to-date.  The other methods (method <= 0) will only update
out-of-date views.
...
(
    char   *sbf_name,                           /* <I>  Name of the sbf file              */
...
char sbf_name[SYMBOL_FILE_NAME_SIZE+1]  /* <I>
                                        file to be set
                                        */
);
...
/*************************************************************************
Sets as current the UG default symbol font
definition file.
...
char sbf_name[SYMBOL_FILE_NAME_SIZE+1]  /* <O>
...
char symbol_name[SYMBOL_FILE_NAME_SIZE+1] ,/* <I>
                                           symbol name
                                           */
double symbol_factor ,/* <I>
                      symbol factor
                      */
double symbol_anchor[3] ,/* <I>
                         model space x,y,z of
                         anchor point
                         */
double symbol_orient[3] ,/* <I>
                         model space x,y,z of
                         orientation point
                         */
int num_objects ,/* <I>
                 ...
                 */
tag_t * object  /* <I,len:num_objects> num_objects
                list of object tags
                */
);
/*************************************************************************
...
/* uf5522 set drafting parameters to */
/****************************************************************************
...
/* uf5524 set drafting display parameters */
/****************************************************************************
...
The 23.UG file must be accessible to the program calling this
...
The 23.UG file must be accessible to the program calling that
subroutine. The maximum number of characters per row is 132.  The
maximum number of rows is 50. Subject to the above, the maximum
number of characters in the note is 650.
...
must be called first to initialize UG/Open API for
...
extern UFUNEXPORT int UF_DRF_ask_embedded_uds_font_info(
    ...
    char                 *symbol_name,    /* <O>  Name of Symbol Font      */
...
Return:
        ...
        Possible return codes can include the following
            ...
            UF_DRF_ERR_INVALID_EXTENSION - The specified Extension type is not
                                           supported
            UF_err_program_not_initialized - UG/Open API has not been
                                             initialized
            UF_DRF_null_object_structure
...
extern UFUNEXPORT int UF_DRF_create_custom_symbol_instance(
    UF_DRF_custom_symbol_p_t symbol_definition, /*<I> Structure containing
                                                      data necessary for
...
/* uc5550 read the drafting object creation - replaced by
                                              UF_DRF_ask_object_preferences */
extern UFUNEXPORT void uc5550(tag_t np1, int *ir2, double *rr3, char *cr4, char *cr5);

/* uc5551 regenerate a drafting object with - replaced by
                                              UF_DRF_set_object_preferences */
extern UFUNEXPORT int uc5551(tag_t np1, int *ip2, double *rp3, char *cp4, char *cp5);
...
extern UFUNEXPORT void uc5530(int ip1, tag_t np2, int ip3, int ip4, tag_t np5, int ip6,
                   int ip7, char *cp8, int ip9, char cp10[][133], double *rp11,
                   tag_t *nr12);

/* uc5531 create a perpendicular dimension */
extern UFUNEXPORT void uc5531(tag_t np1, tag_t np2, int ip3, int ip4, char *cp5, int ip6,
                   char cp7[][133], double *rp8, tag_t *nr9);

/* uc5532 create angular dimension */
extern UFUNEXPORT void uc5532(int ip1, tag_t np2, int ip3, tag_t np4, int ip5, char *cp6,
                   int ip7, char cp8[][133], double *rp9, tag_t *nr10);

/* uc5533 create an arc length, radius, diameter, or hole */
extern UFUNEXPORT void uc5533(int ip1, tag_t np2, char *cp3, int ip4, char cp5[][133],
                   double *rp6, tag_t *nr7);

/* uc5534 create a concentric circle dimension */
extern UFUNEXPORT void uc5534(tag_t np1, tag_t np2, char *cp3, int ip4, char cp5[][133],
                   double *rp6, tag_t *nr7);
...
extern UFUNEXPORT void uc5541(int ip1, char cp2[][133], double *rp3, int ip4, tag_t np5,
                   double *rp6, tag_t *nr7);

/* uc5542 create an id symbol */
extern UFUNEXPORT int uc5542(int ip1, char *cp2, char *cp3, double *rp4, int ip5, int ip6,
                  tag_t np7, double *rp8, tag_t *nr9);

/* uc5543 create a form and positional */
extern UFUNEXPORT void uc5543(int ip1, char cp2[][133], double *rp3, int ip4, int ip5,
...
/*===========================================================================
  ...
  Unigraphics callers which need the UG/Open API (e.g. translators)
