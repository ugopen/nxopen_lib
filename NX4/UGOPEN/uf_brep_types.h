...
UG/Open definitions for UF_BREP specific structures, typedefs, and 
macro definitions.
...
#include <uf_defs.h>         /* Common to all UG/Open, e.g., tag_t, NULL_TAG */
#include <uf_error_bases.h>  /* For UF error codes not specific to a module. */
#include <uf_forgeo.h>       /* FORGEO support */


#ifdef __cplusplus    /* Block start for C++ source files */
extern "C" {
#endif
...
struct UF_BREP_conic_s   /* UF_CURVE-like */
{
...
*/
struct UF_BREP_spline_s   /* UF_CURVE-like */
{
...
/* This structure specifies a tagged UG object to be used in a topology. When 
   these objects are attached, they are no longer available as a separately 
   tagged UG object. 
*/
struct UF_BREP_tagged_object_s
{
    tag_t  object;  /* Unigraphics tag of a curve or suface object. */
};
...
/* User data (C union) that only UG/Open programmer uses. (Although 
...
UF_BREP_ask_identifier. Vertices, fins, and shells do not have UG tags, but do 
...
that shapes the surface to match the 3D edge. On output from UG, not all edges 
...
On input to UG, SP curves and 3D curves may be specified for the same edge. All
...
struct UF_BREP_topology_s
{
    UF_BREP_topo_type_t       type;        /* See UF_BREP_topo_type_s, e.g., 
                                           ...
                                           */
    tag_t                     tag;         /* UG tag, except for shells, fins,
...
* of geometry in a UG body to the geometry the caller receives.
*
...
struct UF_BREP_mapping_s
{
    int                   count; /* Mapping pair count:  source[count],
                                 extracted[count]
                                 */
    UF_BREP_geom_type_t  *source;/* UG body geometry types. */
...
struct UF_BREP_state_s
{
    ...
    UF_BREP_topology_t *item;       /* UF_BREP_topology_t entity having the
                                    ...
                                    */ 
    tag_t               ug_tag;     /* UG entity have the problem, if any.
                                    Some topology entities do not
                                    have UG tags, e.g., vertices.
                                    */
