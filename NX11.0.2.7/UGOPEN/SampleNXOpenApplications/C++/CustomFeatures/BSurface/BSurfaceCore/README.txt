...
-------------------------------------------------------------------------------
        Demonstration example for custom feature B-Surface Core code.
-------------------------------------------------------------------------------
...
--------------
    ...
    3. BSurfaceCore_CPP_ReadMe.txt
	
 The above files are located at $UGII_BASE_DIR/ugopen/SampleNXOpenApplications/C++/CustomFeatures/BSurface/BSurfaceCore
...
------------
  ...
  Windows
  -------
   1. Using Visual Studio - create a new project with name "BSurfaceCore" using the NX Open C++ Wizard.
   ...
   3. Add the BSurfaceCore.cpp to Source Files by selecting the Project pull down and select "Add Existing Item...". Select the file 
      using the popup window.

   4. Build the internal image by going to the Build menu and selecting either 
      "Build Solution" or "Rebuild Solution".
 
   This will create an executable called BSurfaceCore.dll in the Debug directory.
   ...
   The uflink tool can also be used to create the executable by following these steps:
    ...
    ufcomp BSurfaceCore.cpp
      uflink UFDLL=BSurfaceCore.dll BSurfaceCore.obj

   This will create BSurfaceCore.dll in the current directory.
  ...
  -----
   ...
   This will create BSurfaceCore.so in the current directory.

----------------------------
Settings before Launching NX
---------------------------- 
The executable (BSurfaceCore.dll for Windows or BSurfaceCore.so for Linux) and CustomFeatureConfiguration.xml must be placed in any one of the following locations mentioned below:
...
-----------------------
    ...
    To execute above created executable please follow steps in BSurfaceUI example which is loacted at $UGII_BASE_DIR/ugopen/SampleNXOpenApplications/C++/CustomFeatures/BSurface/BSurfaceUI
