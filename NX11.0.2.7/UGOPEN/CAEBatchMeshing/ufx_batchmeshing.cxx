...
typedef struct UF_UG_BatchMesh_Parms_s
{
    ...
    UF_SF_ugs_tet_mesh_fillet_type_option_t   fillet_type;   /* Fillet type */
...
#define   UF_UG__BatchMesh_MESHParams(pzParams)     \
    ...
    (pzParams)->fillet_type = UF_SF_UGS_TET_MESH_FILLET__all; \
    ...
    (pzParams)->target_minimum_element_edge_length = true; \
...
static int do_Batch_Meshing__main ( UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams )
{
    ...
    try
    {
        ...
        if (pzBatchRunParams->Solver_Name && !strcmp(scSolverName.GetLocaleText(), scCommandLineSolverName ))
        {
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, 
        ...
        for ( i=0; i<nCAEBodies; i++)
        {
            ...
            else
            {
                ...
                ================================================== */
                write_mesh_quality_report ( (char*)scSolverName.GetUTF8Text(), scMeshType, mesh_tag, pzBatchRunParams );
            }
                             ...
                             tag_t* mesh_tag )
{
    ...
    tag_t partTag = workFemPart->Tag();
    CAE::MeshCollector *nullCAE_MeshCollector(NULL);
    mesh2dBuilder1->ElementType()->DestinationCollector()->SetElementContainer(nullCAE_MeshCollector);
    ...
    destinationCollectorBuilder1->SetElementContainer(nullCAE_MeshCollector);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("move nodes off geometry bool", mesh_2d_params.move_nodes_off_geometry);
    
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("max quad warp option bool", mesh_2d_params.max_warp_toggle);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh transition bool", (mesh_2d_params.mesh_transition != 0));
                              ...
                              tag_t* mesh_tag )
{
    ...
    tag_t partTag = workFemPart->Tag();
    CAE::MeshCollector *nullCAE_MeshCollector(NULL);
    mesh3dTetBuilder1->ElementType()->DestinationCollector()->SetElementContainer(nullCAE_MeshCollector);
    ...
    mesh3dTetBuilder1->PropertyTable()->SetIntegerPropertyValue("fillet num elements", mesh_3d_params.fillet_num_elem);
    ...
    mesh3dTetBuilder1->PropertyTable()->SetScalarWithDataPropertyValue("volume mesh size variation", mesh_3d_params.tet_mesh_size_variation, nullUnit);
                         ...
                         UF_SF_ugs_2d_mesh_parms_p_t pzmesh_2d_params)
{
    ...
    *scElementName = (char* )UF_allocate_memory(MAX_FSPEC_BUFSIZE*sizeof(char), &mem_error);
    ...
    /* Start processing message */
        ...
        pzmesh_3d_params->tet_mesh_size_variation    = zMesh_params.tet_mesh_size_variation;
        pzmesh_3d_params->mesh_transition            = zMesh_params.mesh_transition_ON_OFF;
        ...
        pzmesh_2d_params->mesh_transition            = zMesh_params.mesh_transition_ON_OFF; 
        ...
        {
            pzmesh_3d_params->fillet_toggle = true;
            pzmesh_3d_params->fillet_type = zMesh_params.fillet_type;
            pzmesh_3d_params->fillet_num_elem = zMesh_params.elem_per_quarter_round;
            pzmesh_3d_params->fillet_min_rad = zMesh_params.min_fillet_radius;
            pzmesh_3d_params->fillet_max_rad = zMesh_params.max_fillet_radius; 
        }
        else 
            pzmesh_3d_params->fillet_toggle = false;
        ...
        if ( UF_UG__BatchMesh_Is2DMesh(zMesh_params.scElemType))
        {
            ...
            if (element_type == UF_SF_TRIA3 || element_type == UF_SF_TRIA6)
                pzmesh_2d_params->mesh_transition = false;
        ...
        if (pzmesh_3d_params->attempt_mapped_meshing == (int)true)               
        ...
        if (pzmesh_3d_params->two_element_through_thickness_toggle == (int)true)       
        ...
        if (pzmesh_3d_params->auto_fix_failed_elements_toggle == (int)true)       
        ...
        if (pzmesh_2d_params->geometry_tolerance_toggle == (int)true ||
            pzmesh_3d_params->geometry_tolerance_toggle == (int)true)
        {           
        ...
        if (pzmesh_3d_params->edge_merge_toggle==(int)true)
        {
        ...
        write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t   Small Feature Size = %f %% Element Size\n",
                             pzmesh_3d_params->small_feature );
        
        if (pzmesh_2d_params->target_minimum_element_edge_length==(int)true)
        {
        ...
        if (pzmesh_2d_params->multi_block_decomposition==(int)true)
        {
        ...
        if (pzmesh_2d_params->move_nodes_off_geometry == (int)true)
        {
        ...
        if (pzmesh_2d_params->max_warp_toggle == (int)true)
        {
        ...
        if ( pzmesh_3d_params->fillet_toggle==(int)true)
        {
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t   Fillet Toggle :  ON\n");
            if (pzmesh_3d_params->fillet_type==UF_SF_UGS_TET_MESH_FILLET__all)
                write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t     Fillet Type = ALL\n");
            else if (pzmesh_3d_params->fillet_type==UF_SF_UGS_TET_MESH_FILLET__inside)
                write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t     Fillet Type = Inside\n");
            else if (pzmesh_3d_params->fillet_type==UF_SF_UGS_TET_MESH_FILLET__outside)
                                  ...
                                  UF_UG_BatchMesh_Parms_t           *pzMesh_params)
{
    ...
    char                           scFilletType[MAX_FSPEC_BUFSIZE]={' '};
            ...
            else
            {
                ...
                if ( meshParamReader.Get_fillet_type(scFilletType))
                {
                    ...
                    if ( meshParamReader.fStringHasPreFix(scFilletType,"All") )
                        pzMesh_params->fillet_type = UF_SF_UGS_TET_MESH_FILLET__all;
                    else if ( meshParamReader.fStringHasPreFix(scFilletType,"Inside") )
                        pzMesh_params->fillet_type = UF_SF_UGS_TET_MESH_FILLET__inside;
                    else if ( meshParamReader.fStringHasPreFix(scFilletType,"Outside") )
                        pzMesh_params->fillet_type = UF_SF_UGS_TET_MESH_FILLET__outside;
                                       ...
                                       UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams)
{
    ...
    if ( fUse_2D_params==TRUE )
    {
        ...
        /* Mesh transition */
        if (mesh_2d_parms.mesh_transition == (int)true && ((!strcmp((scMeshType), "QUAD_4")) ||
            (!strcmp((scMeshType), "QUAD_8"))))
        ...
        if (mesh_2d_parms.max_included_angle_quad_toggle == (int)true)
        {
            ...
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for quad = %f\n", 
                mesh_2d_parms.max_included_angle_quad_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for quad Toggle = OFF\n");

        if (mesh_2d_parms.min_included_angle_quad_toggle == (int)true)
        {
            ...
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for quad = %f\n",
                mesh_2d_parms.min_included_angle_quad_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for quad Toggle = OFF\n");

        if (mesh_2d_parms.max_included_angle_tria_toggle == (int)true)
        {
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria Toggle = ON\n");
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria = 5f\n",
                mesh_2d_parms.max_included_angle_tria_value);
        }
        else
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Max Angle for tria Toggle = OFF\n");

        if (mesh_2d_parms.min_included_angle_tria_toggle == (int)true)
        {
            ...
            write_info_fprintf(true, UF_UG__BatchMesh_LOG, "   Min Angle for tria = %f\n",
                mesh_2d_parms.min_included_angle_tria_value);
        }
        else
                                        ...
                                        int  *pfDisplayCommandHelp)
{
    ...
    char     command_help[ MAX_FSPEC_BUFSIZE+1 ]={""},      /* Command help */    
             param_file[ MAX_FSPEC_BUFSIZE+1 ]={""},        /* Parameter file name */
             criteria_file[ MAX_FSPEC_BUFSIZE+1 ]={""},     /* Criteria file name */
             template_file[ MAX_FSPEC_BUFSIZE+1 ]={""},
             part_file[ MAX_FSPEC_BUFSIZE+1 ]={""},         /* Part filename storing  */

             solver_name[ MAX_FSPEC_BUFSIZE+1 ]={""},       /* Solver name */
             out_no_idealized[ MAX_FSPEC_BUFSIZE+1 ]={""},  /* Output opt: no idealized */
             out_fem[ MAX_FSPEC_BUFSIZE+1 ]={""},           /* Output opt: FEM file */
             out_jt[ MAX_FSPEC_BUFSIZE+1 ]={""},            /*             JT file */
             out_nastran[ MAX_FSPEC_BUFSIZE+1 ]={""},       /*             NASTRAN file */           
             run_key_word[ MAX_FSPEC_BUFSIZE+1 ]={""};      /* Run key word */
    ...
    *pfDisplayCommandHelp = false;
    ...
    /*Template name */
    if (uc4620( UF_UG_BATCH_MESH__PARM_template_file, template_file)<=0)
    {
        pzBatchRunParams->fTemplateFile = false;
        strcpy(template_file,"");
    }
    else
    {
        pzBatchRunParams->fTemplateFile = true;
        strcpy(pzBatchRunParams->template_file_name, template_file);
    }
