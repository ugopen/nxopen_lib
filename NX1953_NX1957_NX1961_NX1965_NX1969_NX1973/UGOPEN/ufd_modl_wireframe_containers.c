...
(
  ...
  int        n_all_edges ,
  const tag_t * all_edges ,
  tag_t    * start_edge1 ,
  tag_t    * start_edge2
)
{
  int      inx = 0 , vcount1 = 0, vcount2 = 0;
  tag_t    lc_edge1 = all_edges != NULL?all_edges[0]:NULL_TAG, lc_edge2 = NULL_TAG;
...
static void do_ugopen_api (void)
{
  ...
  UF_SC_EDGE_tangent_data_t   tangent_edge_data[4] = { 0 };
...
/* Create a wireframe container for all the tangent edges */
  ...
  UF_MODL_init_sc_input_data (rule_types[1], (UF_SC_input_data_t *)&rules[1]);
...
/* Create another body and add two more rules to the container */
  ...
  UF_MODL_init_sc_input_data(rule_types[2], (UF_SC_input_data_t *)&rules[2]);
  ...
  UF_MODL_init_sc_input_data(rule_types[3], (UF_SC_input_data_t *)&rules[3]);
