...
public class NXOpenRemotingService
{
    ...
    //UI.GetUI needs to be called on the main thread, not the created background thread in Start()
    //GetUI will create the UI object, if not already created, which must be done a the main thread
