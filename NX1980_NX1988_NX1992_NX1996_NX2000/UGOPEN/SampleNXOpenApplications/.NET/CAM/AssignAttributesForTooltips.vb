...
Module AssignMyAttributes
    ...
    Private thePicturePath As String = Environment.GetEnvironmentVariable("UGII_BASE_DIR") + "\mach\resource\template_part\images\"
            
    ' File type for images
    Private thePictureFileType As String = ".bmp"
    Private thePictureTextExtension = ".1x.white"
    ...
    Function FindObjectAndAssignAttribute (ByVal rowData As TooltipData)
        ...
        If camObject IsNot Nothing Then
            ...
            If Not String.IsNullOrWhiteSpace(rowData.camPicture) Then  
                If File.Exists(thePicturePath + rowData.camPicture + thePictureTextExtension + thePictureFileType)
                   AssignAttribute (camObject, "CAM_PICTURE", thePictureFolder + rowData.camPicture + thePictureFileType)
                Else If rowData.camPicture.Contains(thePictureFileType)
                  AssignAttribute (camObject, "CAM_PICTURE", rowData.camPicture)
                Else
                   AssignAttribute (camObject, "CAM_PICTURE", rowData.camPicture + thePictureFileType)
                End If
            Else
