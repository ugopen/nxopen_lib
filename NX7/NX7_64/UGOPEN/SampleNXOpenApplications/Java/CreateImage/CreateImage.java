...
*/            fileName.setText(Dir+java.io.File.separator+strDefaultFile+fileExt);
        ...
        else
        {
            ...
            //strArrFileName=strDefaultFile.split(java.io.File.separator);
        ...
        try
        {
            ...
            if(event.getActionCommand().equals("BMP" ) )
            {
                if(isWindowsPlatform())
                {
                    ...
                    fileExt=".bmp";
                    JOptionPane.showMessageDialog(null,"Please select any of above mentioned file extention","Invalid file extention",JOptionPane.INFORMATION_MESSAGE);
                }
              }
...
/*                        if(isWindowsPlatform() )
                            ...
                            fileName.setText(Dir+"/"+strDefaultFile+fileExt);
*/                        fileName.setText(Dir+java.io.File.separator+strDefaultFile+fileExt);
//                        theUFSession.ui().writeListingWindow("\n"+Dir+"\\"+strDefaultFile+fileExt+"\n");
                        theUFSession.ui().writeListingWindow("\n"+Dir+java.io.File.separator+strDefaultFile+fileExt+"\n");
                        checkFileName();
                    }
       ...
       try
        {
            ...
            if(!(fileName.getText().equals("")))
            {
                ...
                checkFileName();
                String strDefaultFile1;
/*                if(isWindowsPlatform() )
                    strDefaultFile1=Dir+"\\"+strDefaultFile;
                else
                    strDefaultFile1=Dir+"/"+strDefaultFile;
*/                strDefaultFile1=Dir+java.io.File.separator+strDefaultFile;

                theUFSession.ui().writeListingWindow("\nFile:"+strDefaultFile+"       Format:"+imageFormat+"   backgroundColor:"+backgroundColor+"\n");
/* Earlier the argument to createImage function was strDefaultFile1. But this was causing the error as the file name was not correctly
found out. Hence passing the argument as strDefaultFile only which has only the file name. It is working now. */
                theUFSession.disp().createImage(strDefaultFile,imageFormat,backgroundColor);
                theUFSession.ui().writeListingWindow("\nFile "+strDefaultFile1+fileExt+ " created successfully ");
/* In the above, passing strDefaultFile1, because it is printing the absolute file location. */
    ...
    protected void processWindowEvent(WindowEvent e)
    {
        ...
        {
            int exit = JOptionPane.showConfirmDialog(this, "Do you want to exit Snag Image?","CLOSE?",JOptionPane.YES_NO_OPTION );
            if (exit == JOptionPane.YES_OPTION)
            {
                cancelDialog();
                setVisible(false);
