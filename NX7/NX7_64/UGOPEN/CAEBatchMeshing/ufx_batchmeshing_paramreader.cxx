...
/* =========================================================
   ...
   ========================================================= */
static int CountNonBlank ( char *scString );
static int CountBlank ( char *scString );

paramReader::paramReader( void ): m_fp_xml(0)
                                    

{
    /* Set everything to "Not set", i.e., false */


    /* === MESH data === */                   
    fSET__element_size = false;
    element_size = 0.0;                    

    fSET__element_type = false;
    strcpy (element_type, "");

    fSET__midnode_option = false;
    strcpy (midnode_option, "");

    fSET__max_mesh_jacobian = false;
    max_mesh_jacobian = 0.0;    

    fSET__surf_mesh_size_variation = false;
    surf_mesh_size_variation = 0.0;           

    fSET__tet_mesh_size_variation = false;
    tet_mesh_size_variation = 0.0; 

    fSET__small_feature_size = false;
    small_feature_size = 0.0;

    fSET_mesh_transition_ON_OFF = false;
    mesh_transition_ON_OFF = 0;

    fSET_2D_mesh_method = false;
    strcpy(mesh_method_2D, "");


    /* ==== Merge Edge data ===== */
    fSET__merge_edge_ON_OFF_Toggle = false;
    merge_edge_ON_OFF_Toggle = 0;

    fSET__merge_edge_angle_value = false;
    merge_edge_angle_value = 0.0;


    /* ==== Fillet data ==== */
    fSET__fillet_type = false;
    strcpy(fillet_type, "");

    fSET__min_fillet_radius = false;
    min_fillet_radius = 0.0;

    fSET__max_fillet_radius = false;
    max_fillet_radius = 0.0;

    fSET__elem_per_quarter_round = false;
    elem_per_quarter_round = 0;


    /* ==== Mesh Criteria Parameters === */
    fSET__tet_collapse = false;
    tet_collapse = 0.0;

    fSET__aspect_ratio = false;
    aspect_ratio = 0.0;

    fSET__jacobian = false;
    jacobian = 0.0;
    ...
    fSET__2D_min_angle = false;
    min_angle_2D = 0.0;
    
    fSET__2D_max_angle = false;
    max_angle_2D = 0.0;
...
int  paramReader::Get_element_size (double *dElem_Size)
{
    if (fSET__element_size==(int)true)
...
int  paramReader::Get_element_type (char *scElem_Type)
{
    if (fSET__element_type==(int)true)
        strcpy(scElem_Type, element_type);
    return fSET__element_type;
}
void paramReader::Set_element_type ( char   scElement_type[PARAMREADER_MAX_LINE_LENGTH] )
{
...
int  paramReader::Get_midnode_option (char *scMid_Node_Opt)
{
    if (fSET__midnode_option==(int)true)
        strcpy(scMid_Node_Opt, midnode_option); 
    return fSET__midnode_option;
}
void paramReader::Set_midnode_option (char scMid_Node_Opt[PARAMREADER_MAX_LINE_LENGTH])
{
    fSET__midnode_option = true;
    strcpy(midnode_option,scMid_Node_Opt);  
}

int  paramReader::Get_surf_mesh_size_variation (double *dSurf_Mesh_Size_Var)
{
    if (fSET__surf_mesh_size_variation==(int)true)
...
int  paramReader::Get_max_mesh_jacobian (double *dMax_Mesh_Jacobian)
{
    if (fSET__max_mesh_jacobian==(int)true)
...
int  paramReader::Get_tet_mesh_size_variation (double *dTet_Mesh_Size_Var)
{
    if ( fSET__tet_mesh_size_variation==(int)true )
...
int  paramReader::Get_edge_match_tolerance  (double *dEdge_Match_Tol)
{
    if ( fSET_edge_match_tolerance==(int)true )
...
int paramReader::Get_attempt_quad_mapping  (int *fQuad_Mapping_ON_OFF_Opt)
{
    if ( fSET_attempt_quad_mapping==(int)true )
...
int paramReader::Get_split_poor_quads  (int *fSplit_Bad_Quad_ON_OFF_Opt)
{
    if ( fSET_split_poor_quads==(int)true )
...
int paramReader::Get_maximum_quad_warp (double *dMax_Quad_Warp)
{
    if ( fSET_maximum_quad_warp==(int)true )
...
int  paramReader::Get_quad_only_option      (int *iQuad_only_option)
{
    if ( fSET_quad_only_option==(int)true )
...
int paramReader::Get_mesh_transition_ON_OFF  (int *fMesh_Transition_ON_OFF_Toggle)
{
    if ( fSET_mesh_transition_ON_OFF==(int)true )
...
void paramReader::Set_2D_mesh_method (char  scMesh_2D_method[PARAMREADER_MAX_LINE_LENGTH])
{
    fSET_2D_mesh_method = true;
    strcpy(mesh_method_2D, scMesh_2D_method); 
}
int paramReader::Get_2D_mesh_method (char *scMesh_2D_method)
{
    if (fSET_2D_mesh_method==(int)true)
...
int  paramReader::Get_small_feature_size (double *dSmall_Feature_Size)
{
...
void paramReader::Set_fillet_type(char scFillet_Type[PARAMREADER_MAX_LINE_LENGTH])
{
    fSET__fillet_type = true;
    strcpy (fillet_type, scFillet_Type);
}
int paramReader::Get_fillet_type(char scFillet_Type[PARAMREADER_MAX_LINE_LENGTH])
{
    if (fSET__fillet_type==(int)true)
...
int paramReader::Get_min_fillet_radius (double *dMin_Fillet_Radius)
{
    if (fSET__min_fillet_radius==(int)true)
...
int paramReader::Get_max_fillet_radius (double *dMax_Fillet_Radius)
{
    if (fSET__max_fillet_radius==(int)true )
...
int paramReader::Get_elem_per_quarter_round (int *iElem_per_Quarter_Round)
{
    if (fSET__elem_per_quarter_round==(int)true)
...
int paramReader::Get_merge_edge_angle_value (double *dMerge_Edge_Angle_Val)
{
    if (fSET__merge_edge_angle_value==(int)true )
...
int paramReader::Get_merge_edge_ON_OFF_Toggle  (int *fMerge_Edge_Toggle_ON_OFF_Opt)
{
    if ( fSET__merge_edge_ON_OFF_Toggle==(int)true )
...
int   paramReader::Get_Tet_Collapse (double *dTet_Collapse)
{
    if (fSET__tet_collapse==(int)true)
...
int   paramReader::Get_Aspect_Ratio (double *dAspect_Ratio)
{
    if (fSET__aspect_ratio==(int)true)
...
int   paramReader::Get_Jacobian (double *dJacobian)
{
    if (fSET__jacobian==(int)true)
        *dJacobian = jacobian;
...
int   paramReader::Get_Warp (double *dWarp)
{
    if (fSET__warp==(int)true)
        *dWarp = warp;
    return fSET__warp;
...
int   paramReader::Get_Skew (double *dSkew)
{
    if (fSET__skew==(int)true)
        *dSkew = skew;
    return fSET__skew;
...
int   paramReader::Get_Taper (double *dTaper)
{
    if (fSET__taper==(int)true)
        *dTaper = taper;
    return fSET__taper;
...
int   paramReader::Get_2D_MinAngle (double *dMinAngle)
{
    if (fSET__2D_min_angle==(int)true)
...
int   paramReader::Get_2D_MaxAngle (double *dMaxAngle)
{
    if (fSET__2D_max_angle==(int)true)
...
int   paramReader::Get_Jacobian_Ratio (double *dJacobian_Ratio)
{
    if (fSET__jacobian_ratio==(int)true)
...
bool paramReader::fStringHasPreFix(char *psInputString,char *psPreFixString)
{
    /*
...
int paramReader::RemoveBlank(void)
{
    int iChar = 0;
...
int paramReader::LocateStartOfValue(char *scTextLine)
{
...
int paramReader::openFile(const char *fn, const char *mode)
{
   ...
   if (NULL == (m_fp_xml=fopen(fn,mode)) )
      {
#if 0
       fprintf(stderr," Cannot open file %s for ",fn); */
       switch (mode[0])
              {
               ...
               case 'a' : fprintf(stderr,"appending");
                          break;
               default  : fprintf(stderr,"some strange mode");
                          break;
              }
       fprintf(stderr," - bye ! \n"); 
#endif       
...
int paramReader::closeFile(void)
{
...
/* =============================================================
   ...
   ============================================================= */
int paramReader::ReadKeyWordAndValue(char *scLineRead, int ValueType,
                                   int *pIntValue, double *pdDoubleValue, char *scTextValue)
{
...
#if 0
    int iChar =0, iCnt=0; 
    char acTOKEN[PARAMREADER_MAX_LINE_LENGTH];

    fgetsSkipComments(m_nextLine, PARAMREADER_MAX_LINE_LENGTH,m_fp_xml);
    if (true == fStringHasPreFix(m_nextLine,"<NUM>"))
    {
        iChar=RemoveBlank();
        iChar += 5;
        iCnt = sscanf(&(m_nextLine[iChar]), "%i %s ",
                        piNum,acTOKEN);

        if ( (EOF == iCnt) || 
            (  2 != iCnt)  ||
            (false == fStringHasPreFix(acTOKEN,"</NUM>")) 
            ) 
        {
            iRet = -1;
        }
...
#if 0
/* Pure comment line any line with # and no valid tag */
int *paramReader::IsPureComment(char *s)
{

   if( s[0]=='#' )
    {
        /* Remove # and see what the next word is */
    }
    else 
        return false
}
...
char *paramReader::fgetsSkipComments(char *s,
                             int iNum,
                             FILE *fp)
{
   char *pRet;
   ...
   while((pRet = fgets(s,iNum,fp))!=0){
       if(true == paramReader::fStringHasPreFix(s,"<COMMENT>")){
                  iCommentState++;
                  continue;
       }
       else if(true  == paramReader::fStringHasPreFix(s,"</COMMENT>")){
                  iCommentState--;
                  continue;
       }
       if(iCommentState > 0) continue;
       else break;
   }
   return(pRet);
}
...
int paramReader::read_params()
{
  ...
  while (fgetsSkipComments(m_nextLine, PARAMREADER_MAX_LINE_LENGTH, m_fp_xml)) 
  {
      ...
      if ( m_nextLine[0]=='#' )
      {
          ...
          else if ( fParmHeadCardRead==false )
...
int paramReader::ReadCriteriaValue( char *scCriteria, 
                                   char *scLineString, 
                                   double *pdDBL_value)
{
