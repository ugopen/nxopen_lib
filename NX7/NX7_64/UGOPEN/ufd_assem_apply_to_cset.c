...
{
/*
   If component is a member of the first set,
      then remove it from the second,
    or
   If component is a member of the second set,
      then remove it from the first,
   else
...
*/
  ...
  UF_ASSEM_is_member_of_cset(data->first_set, component,
                             &result);
  if (result)
  {
  ...
  {
    UF_ASSEM_is_member_of_cset(data->second_set, component,
                               &result);
    if (result)
    {
...
)
{
  ...
  for (inx = 0; inx < component_count; inx++)
  {
    ...
    if (recurse && ifail == 0) ifail = 
                    cycle_components(child_components[inx],
                                     cset, recurse);
    if (ifail != 0) break;
  }
...
*/
  ...
  if (displayed_part != NULL_TAG)
  {
    ...
    {    
      *rtcode = UF_ASSEM_create_cset(displayed_part, top_name, &cset1);
      if (*rtcode == 0)
      {
        *rtcode = cycle_components(root_part_occ, cset1, FALSE);
      }

      if (*rtcode == 0)
      {
        *rtcode = UF_ASSEM_create_cset(displayed_part, low_name, &cset2);
        if (*rtcode == 0)
        {
          *rtcode = cycle_components(root_part_occ, cset2, TRUE);
        }
      }

      *rtcode = UF_ASSEM_ask_all_comp_cset(displayed_part, &all_cset);
      if (*rtcode == 0)
      {
        my_data.first_set = cset1;
        my_data.second_set = cset2;
        *rtcode = UF_ASSEM_apply_to_cset(all_cset, split_sets, &my_data);
      }
    }
  }
