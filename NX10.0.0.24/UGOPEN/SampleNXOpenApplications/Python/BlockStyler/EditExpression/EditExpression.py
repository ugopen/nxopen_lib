    ...
    def apply_cb(self):
        ...
        try:
            if self.expToEdit != None:
                ExpressionValueProperties = self.ExpressionValue.GetProperties()
                editedValue = ExpressionValueProperties.GetDouble("Value")
                self.deletePropertyList(ExpressionValueProperties)
        ...
        try:
            if block == self.FeatureList:
                ...
                if updatedBlock != None:
                    ...
                    subBlockProperties = subBlock.GetProperties()
                    if subBlock != None:
                        featCol = subBlockProperties.GetTaggedObjectVector("SelectedObjects")
                        if len(featCol) > 1:
                            self.newFeatCol[0] = featCol[-1]
                            subBlockProperties.SetTaggedObjectVector("SelectedObjects", self.newFeatCol)
                        elif len(featCol) == 1:
                    ...
                    self.deletePropertyList(subBlockProperties)
                ...
                if updatedBlock != None:
                    ...
                    if strings[0] != "":
                        ...
                        if len(allExpStr) > 0:
                            ExpressionListProperties = self.ExpressionList.GetProperties()
                            ExpressionListProperties.SetStrings("ListItems", allExpStr)
                            self.deletePropertyList(ExpressionListProperties)
                        self.FeatureList.SetItemText(updatedBlock, strings)

            elif block == self.ExpressionList:
                ExpressionListProperties = self.ExpressionList.GetProperties()
                listItems = ExpressionListProperties.GetStrings("ListItems")
                selItems = ExpressionListProperties.GetIntegerVector("SelectedItems")
                self.deletePropertyList(ExpressionListProperties)

                split_string = listItems[selItems[0]].split()

                ExpressionNameProperties = self.ExpressionName.GetProperties()
                ExpressionNameProperties.SetString("Value", split_string[0])
                self.deletePropertyList(ExpressionNameProperties)

                self.expToEdit = self.workPart.Expressions.FindObject(split_string[0])

                ExpressionValueProperties = self.ExpressionValue.GetProperties()
                ExpressionValueProperties.SetDouble("Value", self.expToEdit.Value)
                self.deletePropertyList(ExpressionValueProperties)
