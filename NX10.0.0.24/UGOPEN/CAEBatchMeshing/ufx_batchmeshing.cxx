...
*     
     ...
     =========================
      ...
      ===============================================
         ...
         - Follow standard NX open build and link procvess using either
    ...
    try
    {
        ...
        { 
            ...
            {                
                ...
                catch (const NXException &error)
                {
                    ...
                    {
                        if (LinuxOS)
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//ugii//templates//" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                        else
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\ugii\\templates\\" + pzBatchRunParams->template_file_name), &partLoadStatus2);
                    }
                    catch (const NXException &error)
                    {
        ...
        else
        {
            ...
            catch (const NXException &error)
            {
                ...
                {                    
                    if (LinuxOS)
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//ugii//templates//" + scSolverTemplate), &partLoadStatus2);
                    else
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\ugii\\templates\\" + scSolverTemplate), &partLoadStatus2);
                }
                catch (const NXException &error)
                {
                    if ( error.ErrorCode() != JA_ERR_UNEXPECTED_SUCCESS )
                              ...
                              tag_t* mesh_tag )
{
    ...
    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh transition bool", (mesh_3d_params.mesh_transition != 0));
                         ...
                         UF_SF_ugs_2d_mesh_parms_p_t pzmesh_2d_params)
{
    ...
    *scElementName = (char* )UF_allocate_memory(MAX_FSPEC_BUFSIZE*sizeof(char), &mem_error);
    ...
    /* Start processing message */
        ...
        {
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t  Multi Block Decompostion:  ON\n");            
        }
        else
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t  Multi Block Decompostion :  OFF\n");
