...
/*   switch name                min   handler function          multi seen
    ...
    {"validation_mode",          12,  validation_mode,         false, false,
     "=off|import_from_part|run_validation specifies how to import validation results\n\tthis switch must be positioned before the -assembly switch, -part switch and -dryrun switch.\n\t Default is off"
    },
    {"validation_rule",          12,  validation_rule,         false, false,
    "=<validation rule dataset name> speicifies the dataset used to qualify the validation results to be imported.\n\t this switch must be specified when -validation_mode is not off. \n\t the name should be in CLI format. \n\t validation rule in native file system is only allowed for dry-run mode. "
    },
...
static void dryrun(const char *ignored)
{
    ...
    if( validation_options!= NULL)
    {
        ...
        {
            const char*  warning =  "Even it is under dry-run mode, the utility will still read validation results from each part "
                                                "and compare against validation rule. This could take some time if the target "
                                                "assembly or parts are large or in large amount.";

            message("\nNote: %s\n", warning);
        }
        else if ( validation_options->mode == UF_CLONE_run_validation)
        {
             const char*  warning =  "Even it is under dry-run mode, the utility will still open each part, "
