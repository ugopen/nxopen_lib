...
Module ShapeCreators
    ...
    Public Class KeyholeShapeCreator
        ...
        Public Sub Execute(ByVal dimensionData() As DimensionData) Implements IShapeCreator.Execute
            ...
            ' Null placeholders for subsequent function calls
            Dim nullNXOpen_SketchPattern As NXOpen.SketchPattern = Nothing
            Dim nullNXOpen_NXObject As NXOpen.NXObject = Nothing

            ' Initialize mirror builder & assign a section to mirror
            Dim sectionToMirror1 As NXOpen.Section = Nothing
            Dim sketchMirrorPatternBuilder1 As NXOpen.SketchMirrorPatternBuilder = Nothing
            sketchMirrorPatternBuilder1 = workPart.Sketches.CreateSketchMirrorPatternBuilder(nullNXOpen_SketchPattern)
            sectionToMirror1 = sketchMirrorPatternBuilder1.Section
            
            ' Create array of curves to mirror (must be an array, even for one curve)
            Dim objectsToMirror1(0) As NXOpen.IBaseCurve
            objectsToMirror1(0) = line2
            
            ' Create rules
            Dim curveDumbRule1 As NXOpen.CurveDumbRule = workPart.ScRuleFactory.CreateRuleBaseCurveDumb(objectsToMirror1)
            Dim mirrorRules1(0) As NXOpen.SelectionIntentRule 
            mirrorRules1(0) = curveDumbRule1

            ' Create (dummy) help point
            Dim helpPoint1 As NXOpen.Point3d = New NXOpen.Point3d(0.0, 0.0, 0.0)

            ' Add curves to mirror to the mirror section
            sectionToMirror1.AddToSection(mirrorRules1, line2, nullNXOpen_NXObject, nullNXOpen_NXObject, helpPoint1, NXOpen.Section.Mode.Create, False)

            ' Define what object to mirror about (datum axis, line, ...)
            sketchMirrorPatternBuilder1.DirectionObject.Value = line1

            ' Execute mirroring
            Dim nXObject1 As NXOpen.NXObject = Nothing
            nXObject1 = sketchMirrorPatternBuilder1.Commit()

            ' Retrieve mirrored curves
            Dim mirroredObjects1() As NXOpen.NXObject
            mirroredObjects1 = sketchMirrorPatternBuilder1.GetCommittedObjects()

            ' Clean up mirror code objects
            sketchMirrorPatternBuilder1.Destroy()
            sectionToMirror1.Destroy()
            ...
            Dim sketch_ConstraintGeometry4 As Sketch.ConstraintGeometry = New Sketch.ConstraintGeometry(mirroredObjects1(0), Sketch.ConstraintPointType.None, 0)
            ...
            Dim sketch_ConstraintGeometry7 As Sketch.ConstraintGeometry = New Sketch.ConstraintGeometry(mirroredObjects1(0), Sketch.ConstraintPointType.None, 0)
