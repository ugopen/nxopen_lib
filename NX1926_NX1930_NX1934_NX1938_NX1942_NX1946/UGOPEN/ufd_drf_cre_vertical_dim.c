...
{
   int status = 0;
   static double  point[3] = {8.0, 4.0, 0.0};
   static double origin[3] = {9.0, 6.0, 0.0};
   static char dimtxt[15+1] = "VERTICAL DIM TX";
   ...
   if (!status)
   {
      ...
      status = UF_DRF_create_vertical_dim(&object1, &object2,
               &drf_text, origin, &dimension_tag);
   }

   printf("UF_DRF_create_vertical_dim ");
   if (status)
   {
