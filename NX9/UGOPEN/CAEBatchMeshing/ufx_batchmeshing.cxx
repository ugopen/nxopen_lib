     ...
     =========================
           ...
           ufx_batchmeshing  -?  -h or -H 
            ...
            -solver=<Solver name>           : Solver name values are
                                                  ...
                                                  "NX_THERMAL_OR_FLOW"
                                                  "NX_NASTRAN DESIGN"
                                                  "NX_ELECTRONIC_SYSTEMS_COOLING"
                                                  "NX_SPACE_SYSTEMS_THERMAL"
                                                  "LSDYNA"
                                                  "SC03"
                                                  "NX_MULTIPHYSICS"
                                                  "IDEAS_UNV"
...
* 25-Oct-2010  Keith Hafen     Back out above change
* 21-Apr-2011  Jack Marr       First arg to main should be typed int, not char
* 21-Apr-2011  Jack Marr       Remove unnecessary headers
* 15-Aug-2011  Ashish Das      Support JT output from mesh.
* 06-Jun-2012  Jimmy Pan       Revised to compatible with new element quality check
* 24-Aug-2012  Shyam Nuggehalli Changes for ARCH9354 Internationalizatio
* 07-Nov-2012  Bhanu Peddi     CAE 1462 - Batch Mesher was rewritten
* 20-Nov-2012  Bhanu Peddi     CAE 1462 - Added Mesh Control Geometry Assignment Module.
* 28-Nov-2012  Bhanu Peddi     CAE 1462 - Added a body_type as a parameter and replace existing FEM.
* 30-Nov-2012  Bhanu Peddi     CAE 1462 - FEM was not being saved due to previous change and added template to help.
* 03-Dec-2012  Shyam Nuggehalli Changes for ARCH9354 Internationalization All User Text
* 04-Dec-2012  Bhanu Peddi     Fixed PR6808091,PR6808123,PR6808106,PR6808131,PR6808163 and PR 6808170.
* 04-Dec-2012  Bhanu Peddi     Fixed PR6808106 - used strcmp instead prefix check.
* 04-Dec-2012  Bhanu Peddi     Fixed PR6810199,PR6809655 - Outputing dat files with -dat option batchmesher 
                               failing on different solver languages.
* 13-Dec-2012  Bhanu Peddi     Fixed SetElementTypeName to take what user had provided in the param file.
* 14-Dec-2012  Shyam Nuggehalli Fixed incorrect merge.
* 07-Jan-2013  Bhanu Peddi     Added Suppress Hole Parameter to Batchmesher.
* 08-Jan-2013  Bhanu Peddi     Added support for SC03,LSDYNA,NX Multiphysics, NX design and NX space systems and cooling.
* 11-Jan-2013  Bhanu Peddi     CAE 1462 - Added more comments, added replace master when template provided and PR 6810236.
* 11-Jan-2013  Bhanu Peddi     Fixed error in getting an element name.
* 14-Jan-2013  Bhanu Peddi     Fixed PR6824569,PR6824534,PR6824468.
* 15-Jan-2013  Bhanu Peddi     Added support for more language solver decks.
* 15-Jan-2013  Bhanu Peddi     Tweaked batchmesher to simple way of doing things.
* 22-Jan-2013  Bhanu Peddi     Added descriptive messages during template finding.
* 22-Jan-2013  Bhanu Peddi     Check the right folder in UGII_BASE_DIR.
* 23-Jan-2013  Bhanu Peddi     add ugii to the path to find it in right template folder.
* 23-Jan-2013  Bhanu Peddi     Removed a error message when printing deck file.
* 14-Feb-2013  Bhanu Peddi     Changed display of help in batchmesher.
* 20-Feb-2013  Bhanu Peddi     New way of saving the FEM in batch mode.
* 07-May-2013  Bhanu Peddi     PR 6885534 - Nastran export from batchmesher fails
* 09-May-2013  Bhanu Peddi     Batch mesher still appears as version 1.0
* 21-May-2013  Bhanu Peddi     Added Target Minimum Element Edge length
* 20-Jun-2013  Bhanu Peddi     Replaced CommitMesh with CommitWithPartialMesh in 3d Tet Mesh on Body
* 24-Jun-2013  Bhanu Peddi     Replaced wording of embedded
* 01-Jul-2013  Bhanu Peddi     assign material to the body
* 03-Jul-2013  Bhanu Peddi     PR6908788 - LINUX64: Batchmesher is not working if a Template is not specified
* 07-Jul-2013  Bhanu Peddi     Missed a merge on previous change
*$HISTORY$
*****************************************************************************/
...
#include <uf_assem.h>
#include <ja_common_error_codes.h>
    ...
    if ( pzBatchRunParams->fOutputDAT==(int)true )
    {
        ...
        if ( iRc==0 &&  new_sim_tag!=NULL_TAG )
        {
            ...
            if (strlen(scPartDirectoryPath))
            {
                ...
                caeDataContainer1->AddInteger("units", 4);
...
static int do_Batch_Meshing__main ( UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams )
{
    ...
    try
    {
        ...
        char                           scCommandLineSolverName[MAX_LINE_LENGTH] = {' '};
        char                           scSolverTemplate[MAX_LINE_LENGTH] = {' '};
        char                           scCommandLineAnalysisType[MAX_LINE_LENGTH] = {' '};
        ...
        { 
            ...
            {                
                ...
                {
                    if (LinuxOS)
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"//" + pzBatchRunParams->template_file_name).GetLocaleText(), &partLoadStatus2);
                    else
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"\\" + pzBatchRunParams->template_file_name).GetLocaleText(), &partLoadStatus2);
                }
                catch (NXException &error)
                {
                    ...
                    {
                        if (LinuxOS)
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//ugii//templates//" + pzBatchRunParams->template_file_name).GetLocaleText(), &partLoadStatus2);
                        else
                            basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\ugii\\templates\\" + pzBatchRunParams->template_file_name).GetLocaleText(), &partLoadStatus2);
                    }
                    catch (NXException &error)
                    {
        ...
        else
        {
            ...
            {
                if (LinuxOS)
                    basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"//" + scSolverTemplate).GetLocaleText(), &partLoadStatus2);
                else
                    basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_TEMPLATE_DIR"))+"\\" + scSolverTemplate).GetLocaleText(), &partLoadStatus2);
            }
            catch (NXException &error)
            {
                ...
                {                    
                    if (LinuxOS)
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"//ugii//templates//" + scSolverTemplate).GetLocaleText(), &partLoadStatus2);
                    else
                        basePart2 = theSession->Parts()->OpenBaseDisplay((theSession->GetEnvironmentVariableValue(NXString("UGII_BASE_DIR"))+"\\ugii\\templates\\" + scSolverTemplate).GetLocaleText(), &partLoadStatus2);
                }
                catch (NXException &error)
                {
                    if ( error.ErrorCode() != JA_ERR_UNEXPECTED_SUCCESS )
        ...
        if ( partLoadStatus2->NumberUnloadedParts() != 0 )
        {                
        ...
        femPart2->FinalizeCreation(part, Part_part_name, CAE::FemPart::UseBodiesOptionVisibleBodies, bodies1, femSynchronizeOptions1, scSolverName, scAnalysisType, description1);
        ...
        for ( i=0; i<nCAEBodies; i++)
        {
            ...
            else
            {
                ...
                ================================================== */
                write_mesh_quality_report ( (char*)scSolverName.GetLocaleText(), scMeshType, mesh_tag, pzBatchRunParams );
            }
        ...
        ========================================== */
        if ( pzBatchRunParams->fOutputDAT==(int)true )
            export_nastran_file ( tag_part_to_mesh, new_fem_tag, scSolverName.GetLocaleText(), pzBatchRunParams );
                             ...
                             tag_t* mesh_tag )
{
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mapped mesh option bool", mesh_2d_params.attempt_quad_mapping);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("split poor quads bool", mesh_2d_params.split_poor_quads);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh format to solver bool", mesh_2d_params.format_mesh);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("target minimum element edge length", mesh_2d_params.target_minimum_element_edge_length);

    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh transition bool", mesh_2d_params.mesh_transition);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("quad mesh edge match toggle", mesh_2d_params.edge_match_toggle);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("merge edge toggle bool", mesh_2d_params.edge_merge_toggle);
    ...
    mesh2dBuilder1->PropertyTable()->SetBooleanPropertyValue("suppress hole option bool", mesh_2d_params.suppress_hole_toggle);
                              ...
                              tag_t* mesh_tag )
{
    ...
    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("mapped mesh option bool", mesh_3d_params.attempt_mapped_meshing);
    ...
    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("two elements through thickness bool", mesh_3d_params.two_element_through_thickness_toggle);

    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("mesh transition bool", mesh_3d_params.mesh_transition);

    mesh3dTetBuilder1->PropertyTable()->SetBooleanPropertyValue("remesh on bad quality bool", mesh_3d_params.auto_fix_failed_elements_toggle);
    ...
    *scElementName = (char* )UF_allocate_memory(MAX_FSPEC_BUFSIZE*sizeof(char), &mem_error);
    ...
    /* Start processing message */
        ...
        if (pzmesh_2d_params->edge_match_toggle==(int)true)
        {
            ...
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "\t  Match Edges Tolearnce :  %f\n", 
...
void get_solver_and_template ( UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams, int partunits, char* scSolverName, char* scAnalysisType, char* scSolverTemplate )
{
    ...
    /* Solver name string */
     ...
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_THERMAL_OR_FLOW" ))
     {
         ...
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxThermalFlowMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxThermalFlowEnglish.fem");
     }
     ...
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_ELECTRONIC_SYSTEMS_COOLING" ))
     {
         ...
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxElecSysCoolingMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxElecSysCoolingEnglish.fem");
     }
     else if ( !strcmp(pzBatchRunParams->Solver_Name,"NX_SPACE_SYSTEMS_THERMAL" ))
     {
         ...
         strcpy (scAnalysisType, "Thermal");
         if (partunits == UF_PART_METRIC)
             strcpy (scSolverTemplate, "FemNxSpaceSysThermalMetric.fem");
         else
             strcpy (scSolverTemplate, "FemNxSpaceSysThermalEnglish.fem");
     }
                                                 ...
                                                 UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams)
{
    ...
    else
    {
        ...
        write_info_fprintf ( true, UF_UG__BatchMesh_LOG,  "Error atempting to write JT from mesh: %s   \n\n",JT_MESH_file_Name);
    }
