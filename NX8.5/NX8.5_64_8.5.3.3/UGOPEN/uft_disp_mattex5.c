...
Scenario 4: assign none to all bodies with original cyan_material
...
{
   tag_t material_tag;
   char material_name[ UF_CFI_MAX_FILE_NAME_SIZE];
...
static void do_it(void)
{
    ...
    tag_t   material_tag;
    char lwa_file_name[ UF_CFI_MAX_PATH_NAME_SIZE ]; //256+1
    ...
    { 
        ...
        if( material_tag != 0 &&  material_found <5 &&
            not_added_material(materials_list, (const char*)material_name) &&
    ...
    if(material_found)
    {
        ...
        while (uc1600("Please enter scenario number: 1 (apply pink shoe leather), 2 (apply webbing to facet_bodies), 3 (apply grid_black), or 4(assign none)", input, &input_len) == 5) 
        {
            ...
            switch (scenario_counter)
            {
                ...
                case 4:
                // assign none to all body with original cyan_material
                for(ii=0; ii<body_list_count;ii++)
