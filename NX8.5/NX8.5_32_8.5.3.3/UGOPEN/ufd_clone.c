...
static void process_switches(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if( descriptor != NULL && strcmp( descriptor->switch_name, "revision_rule") == 0)
        {
            for(j = i + 1; j < argc; j++ )
            {
                switch_descriptor_p_t assem_descriptor = match_switch(argv[j]);
                if( assem_descriptor != NULL && (strcmp(assem_descriptor->switch_name, "assembly") == 0 || strcmp( assem_descriptor->switch_name, "revision_rule") == 0 || strcmp( assem_descriptor->switch_name, "part") == 0))
                {
                    default_rev_rule = 1;
                    break;
                }
    ...
    */
    ...
    if (ifail == 0)
    {
        ...
        */
        ...
        {
            if (strcmp(attributes[i].title,"DB_PART_NO") == 0)
